...
export function RulePatternBuilder({ value, onChange }: RulePatternBuilderProps) {
    ...
    return (
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <Button type="button" variant="ghost" size="icon" onClick={toggleMode} title="切换到组合条件">
            <Braces className="h-4 w-4" />
          </Button>
        </div>
        <p className="text-xs text-muted-foreground">
          DSL: <code className="bg-muted px-1 rounded">{buildDSL("simple", condition, compound) || "..."}</code>
        </p>
      </div>
    );
  }
  ...
  const updateChild = (id: string, field: "kind" | "arg", val: string) => {
    const children = compound.children.map((c) => (c.id === id ? { ...c, [field]: val } : c));
  ...
  const addChild = () => {
    const children = [...compound.children, { id: newId(), kind: "block_ip", arg: "" }];
    const g = { ...compound, children };
    setCompound(g);
    emitChange("compound", condition, g);
  };
    ...
    <Card className="border-dashed">
      <CardContent className="p-3 space-y-3">
        <div className="flex items-center gap-2">
          ...
          <Select value={compound.op} onValueChange={(v) => setOp(v as "and" | "or" | "not")}>
          ...
          <Button type="button" variant="ghost" size="sm" onClick={toggleMode} className="text-xs">
            切换简单模式
          </Button>
        </div>
        ...
        {compound.children.map((child) => (
          <div key={child.id} className="flex items-center gap-2">
            ...
            {compound.children.length > 1 && (
              <Button type="button" variant="ghost" size="icon" onClick={() => removeChild(child.id)}>
        ...
        <p className="text-xs text-muted-foreground">
          DSL: <code className="bg-muted px-1 rounded text-[10px] break-all">
