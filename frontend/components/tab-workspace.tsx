...
/** 解析路径对应的标题信息 */
function resolveTab(pathname: string): WorkspaceTab {
...
export function TabWorkspaceProvider({
  ...
  useEffect(() => {
    ...
    const withPinned = seen.has(normalizePath(PINNED_PATH))
      ? deduped
  ...
  useEffect(() => {
    ...
    setTabs((prev) => {
      ...
      if (prev.some((tab) => normalizePath(tab.path) === key)) return prev;
      return [...prev, resolveTab(pathname)];
    });
  }, [pathname]);
  ...
  const closeTab = useCallback(
    (path: string) => {
      if (normalizePath(path) === normalizePath(PINNED_PATH)) return;
      setTabs((prev) => {
  ...
  return (
    <TabWorkspaceContext.Provider value={ctxValue}>
      <div className="flex h-10 items-stretch gap-1 overflow-x-auto border-b bg-card px-2">
        {tabs.map((tab) => {
          const active =
            normalizePath(tab.path) === normalizePath(pathname ?? "");
          const pinned =
            normalizePath(tab.path) === normalizePath(PINNED_PATH);
          return (
            <div
              key={tab.path}
              className={cn(
