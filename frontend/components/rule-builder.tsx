...
import { AlertCircle, CheckCircle2, Code, Eye, Plus, TestTube, Trash2 } from "lucide-react";
...
function parsePattern(raw: string): { mode: "simple" | "compound"; condition: Condition; compound: CompoundGroup } {
  ...
  const fallbackCompound = { op: "and" as const, children: [{ id: newId(), kind: "block_ip", arg: "" }] };

  if (!trimmed) {
    return { mode: "simple", condition: fallbackCondition, compound: fallbackCompound };
  }

  if (trimmed.startsWith("{")) {
    try {
      const obj = JSON.parse(trimmed) as { op?: "and" | "or" | "not"; children?: Array<{ kind?: string; arg?: string }> };
        ...
        return {
          ...
          compound: {
            ...
            children: obj.children.length
              ? obj.children.map((child) => ({ id: newId(), kind: child.kind || "", arg: child.arg || "" }))
...
function buildDSL(mode: "simple" | "compound", condition: Condition, compound: CompoundGroup) {
  if (mode === "simple") {
    if (!condition.kind || !condition.arg) return "";
    return `${condition.kind}:${condition.arg}`;
  }

  const validChildren = compound.children.filter((child) => child.kind && child.arg);
  ...
  return JSON.stringify({
    op: compound.op,
    children: validChildren.map((child) => ({ kind: child.kind, arg: child.arg })),
  });
}
...
export function RuleBuilder({ value, onChange }: RuleBuilderProps) {
  ...
  const [condition, setCondition] = useState<Condition>({ id: newId(), kind: "block_ip", arg: "" });
  const [compound, setCompound] = useState<CompoundGroup>({ op: "and", children: [{ id: newId(), kind: "block_ip", arg: "" }] });
  ...
  function emit(nextMode: "simple" | "compound", nextCondition: Condition, nextCompound: CompoundGroup) {
  ...
  function switchVisualMode(nextMode: "simple" | "compound") {
    ...
    if (nextMode === "compound" && compound.children.length === 0) {
      const nextCompound = { op: "and" as const, children: [{ id: newId(), kind: condition.kind, arg: condition.arg }] };
  ...
  function addChild() {
    const next = { ...compound, children: [...compound.children, { id: newId(), kind: "block_ip", arg: "" }] };
    setCompound(next);
    emit("compound", condition, next);
  }
  ...
  function removeChild(id: string) {
    ...
    const next = { ...compound, children: nextChildren.length ? nextChildren : [{ id: newId(), kind: "block_ip", arg: "" }] };
    setCompound(next);
    emit("compound", condition, next);
  }
  ...
  function updateChild(id: string, field: "kind" | "arg", nextValue: string) {
    const next = {
      ...compound,
      children: compound.children.map((child) => (child.id === id ? { ...child, [field]: nextValue } : child)),
    };
    setCompound(next);
    emit("compound", condition, next);
  }
  ...
  function testRule() {
    ...
    try {
      if (dsl.startsWith("{")) {
        setTestResult({ match: false, message: "复合规则的精确测试依赖后端验证接口。" });
        return;
      }
      ...
      if (kind.includes("_ip")) {
        ...
        match = kind.includes("_regex") ? new RegExp(arg.split(":")[1] || arg).test(testInput.headers) : testInput.headers.toLowerCase().includes(arg.toLowerCase());
        message = match ? "请求头匹配规则" : "请求头不匹配";
      } else if (kind.includes("_query")) {
        match = kind.includes("_regex") ? new RegExp(arg).test(testInput.query) : testInput.query.includes(arg);
        message = match ? "查询参数匹配规则" : "查询参数不匹配";
      } else if (kind.includes("_body")) {
        match = kind.includes("_regex") ? new RegExp(arg).test(testInput.body) : testInput.body.includes(arg);
        message = match ? "Body 匹配规则" : "Body 不匹配";
      }

      setTestResult({ match, message });
    } catch (error) {
      setTestResult({ match: false, message: error instanceof Error ? error.message : "测试失败" });
    }
  }
    ...
    <div className="space-y-5 rounded-lg border border-slate-200 bg-slate-50/80 p-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs leading-5 text-slate-500">支持简单 DSL、复合 JSON 与快速测试，用于生成真实 pattern 字段。</p>
        </div>
        <Button
          type="button"
          ...
          <TabsContent value="visual" className="space-y-5">
            <div className="inline-flex rounded-full border border-slate-200 bg-white p-1">
              <button
                ...
                className={mode === "simple" ? "rounded-full bg-slate-950 px-4 py-2 text-xs font-medium text-white" : "rounded-full px-4 py-2 text-xs font-medium text-slate-500"}
              >
                简单条件
              </button>
              <button
                ...
                className={mode === "compound" ? "rounded-full bg-slate-950 px-4 py-2 text-xs font-medium text-white" : "rounded-full px-4 py-2 text-xs font-medium text-slate-500"}
              >
                复合条件
              </button>
            </div>
            ...
            {mode === "simple" ? (
              <div className="space-y-4">
                    ...
                    <Select value={condition.kind} onValueChange={setSimpleKind}>
                      ...
                      <SelectContent>
                        {groupedKinds.map(([group, items]) => (
                          <div key={group}>
                            <div className="px-2 py-1.5 text-xs font-semibold text-slate-400">{group}</div>
                            {items.map((item) => (
                ...
                <div className="rounded-lg border border-slate-200 bg-white px-4 py-3 text-xs leading-6 text-slate-600">
                  ...
                  <code className="break-all text-[11px]">{simplePreview || "未生成规则"}</code>
                </div>
              </div>
            ) : (
              <div className="space-y-4">
                ...
                <div className="rounded-lg border border-slate-200 bg-white px-4 py-3 text-xs leading-6 text-slate-600">
                  ...
                  <code className="break-all text-[11px]">{compoundPreview || "未生成规则"}</code>
                </div>
              </div>
            )}
