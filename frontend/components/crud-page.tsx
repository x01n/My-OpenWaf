...
export function CrudPage({
  ...
  async function handleDelete() {
    ...
    try {
      await api(`${apiPath}/${deleteTarget[idField]}/delete`, {
        method: "POST",
      })
      ...
      >
        {loading ? (
          ...
          <div className="overflow-hidden rounded-lg border border-slate-200">
            <Table>
              <TableHeader>
                <TableRow className="bg-slate-50 text-xs tracking-[0.16em] text-slate-500 uppercase">
                  <TableHead className="w-16">ID</TableHead>
                  {tableColumns.map((field) => (
                    <TableHead key={field.key}>{field.label}</TableHead>
                  ))}
                ...
                {items.map((item) => (
                  ...
                  >
                    ...
                    {tableColumns.map((field) => (
                      <TableCell
                        key={field.key}
                        className="max-w-[300px] truncate text-sm text-slate-700"
                      >
                        {field.render
