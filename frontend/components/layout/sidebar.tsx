...
import { ChevronDown, ChevronLeft, ChevronRight, LogOut, ShieldCheck } from "lucide-react";
      ...
      <nav className="flex-1 overflow-y-auto px-2 py-3">
        <div className="space-y-0.5">
          {allItems.map((item) => {
            ...
            const expanded = expandedItems.has(item.href) || (hasChildren && active);
                ...
                {!collapsed && hasChildren && expanded && (
                  <div className="ml-4 mt-0.5 space-y-0.5 border-l border-slate-200 pl-3">
                    {item.children!.map((child) => {
                      const childActive = pathname === child.href || (child.exact !== false && pathname?.startsWith(child.href));
                      return (
                        <Link
                          key={child.href}
                          href={child.href}
                        ...
                        >
                          <child.icon className={cn("h-3.5 w-3.5", childActive ? "text-teal-500" : "text-slate-400")} />
