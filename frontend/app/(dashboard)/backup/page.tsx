...
export default function BackupPage() {
  ...
  const summaryItems = fileData
    ? [
        ...
        {
          label: t("backup.certificates"),
          value: fileData.certificates?.length ?? 0,
        },
        { label: t("backup.rules"), value: fileData.rules?.length ?? 0 },
        {
          label: t("backup.ipEntries"),
          value: fileData.ip_list_entries?.length ?? 0,
        },
        {
          label: t("backup.luaPlugins"),
          value: fileData.lua_plugins?.length ?? 0,
        },
      ]
    : []
  ...
  return (
    <div className="space-y-6">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="font-mono text-xs">
                    {fileData.exported_at
                      ? formatDate(fileData.exported_at)
                      : "-"}
                  </span>
                </div>
              </div>
      ...
      <ConfirmDialog
        ...
        description={
          replaceMode
            ? t("backup.confirmReplaceDesc")
            : t("backup.confirmMergeDesc")
        }
