...
export default function DashboardPage() {
  ...
  const runtimeItems: MetricStripItem[] = [
    {
      ...
      rawValue: d.requests_total,
    },
    {
      label: t("dashboard.pv"),
      value: formatNumber(d.status_2xx),
      rawValue: d.status_2xx,
    },
    {
      label: t("dashboard.uniqueIp"),
      value: formatNumber(d.unique_ips),
      rawValue: d.unique_ips,
    },
    {
      label: t("dashboard.attackIp"),
      value: formatNumber(d.attack_ips),
      rawValue: d.attack_ips,
    },
    {
    ...
    {
      label: t("dashboard.uv"),
      value: formatNumber(d.waf_observes),
      rawValue: d.waf_observes,
    },
    {
  ...
  return (
    <div className="space-y-3">
                ...
                <div className="space-y-2.5">
                  <div className="flex justify-between text-sm">
                    <span className="text-muted-foreground">
                      {t("dashboard.totalDetect")}
                    </span>
                    <span className="font-semibold">
                      {formatNumber(d.bot_total_24h)}
                    </span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-muted-foreground">
                    ...
                    <span className="font-semibold text-red-600">
                      {formatNumber(d.bot_blocked_24h)}
                    </span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-muted-foreground">
                    ...
                    <span className="font-semibold text-amber-600">
                      {formatNumber(d.bot_high_risk_24h)}
                    </span>
                  </div>
                  {d.bot_total_24h > 0 && (
                    <div className="pt-1">
                      <div className="h-2 overflow-hidden rounded-full bg-muted">
                        <div
                          ...
                          style={{
                            width: `${Math.min(100, (d.bot_blocked_24h / d.bot_total_24h) * 100)}%`,
                          }}
                        />
                      </div>
                      <p className="mt-1 text-[10px] text-muted-foreground">
                        {((d.bot_blocked_24h / d.bot_total_24h) * 100).toFixed(
                          1
                        )}
              ...
              <CardContent className="px-4 pt-0 pb-3">
                {statsData?.top_ips && statsData.top_ips.length > 0 ? (
                  <div className="space-y-1.5">
                    {statsData.top_ips.map(
                      (
                        item: { client_ip: string; count: number },
                        idx: number
                      ) => (
                        <Link
                          key={item.client_ip}
            ...
            <Card>
              ...
              <CardContent className="px-4 pt-0 pb-3">
                {statsData?.top_paths && statsData.top_paths.length > 0 ? (
                  <div className="space-y-1.5">
                    {statsData.top_paths.map(
                      (item: { path: string; count: number }, idx: number) => (
                        <Link
                          key={item.path}
      ...
      {/* 运行时信息 - 始终显示 */}
      <Card>
        <CardContent className="flex flex-wrap items-center gap-5 px-4 py-3">
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className="font-medium">
              {Math.floor(d.uptime_sec / 86400)}d{" "}
              {Math.floor((d.uptime_sec % 86400) / 3600)}h{" "}
              {Math.floor((d.uptime_sec % 3600) / 60)}m
            </span>
          </div>
          <div className="flex items-center gap-2 text-sm">
