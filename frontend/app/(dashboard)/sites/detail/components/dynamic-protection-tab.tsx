...
export function DynamicProtectionTab({ site }: DynamicProtectionTabProps) {
  ...
  // 站点级三态覆盖状态
  const [master, setMaster] = useState<TriState>(() =>
    toTriState(site.dynamic_protection_enabled)
  )
  const [html, setHtml] = useState<TriState>(() =>
    toTriState(site.dynamic_html_enabled)
  )
  const [js, setJs] = useState<TriState>(() =>
    toTriState(site.dynamic_js_enabled)
  )
  const [jsMode, setJsMode] = useState<"all" | "paths">(() =>
    site.dynamic_js_mode === "paths" ? "paths" : "all"
  )
  const [jsPaths, setJsPaths] = useState<string[]>(() =>
    parseJSPaths(site.dynamic_js_paths)
  )
  const [ttl, setTtl] = useState<string>(() =>
    site.dynamic_decrypt_cache_ttl == null
      ? ""
      : String(site.dynamic_decrypt_cache_ttl)
  )
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base">
            {t("sites.detail.dynOverride.title")}
          </CardTitle>
          <p className="text-xs text-muted-foreground">
            {t("sites.detail.dynOverride.desc")}
          </p>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          >
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="space-y-0.5">
                <Label className="font-normal">
                  {t("sites.detail.dynOverride.html")}
                </Label>
                <p className="text-xs text-muted-foreground">
            ...
            <div className="space-y-1">
              <Label className="text-xs font-medium">
                {t("sites.detail.dynOverride.ttl")}
              </Label>
              <div className="flex items-center gap-2">
                <Input
                  type="number"
