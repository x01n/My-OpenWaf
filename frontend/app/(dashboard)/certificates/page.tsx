...
import { FileKey2, Plus, Trash2, Eye, Upload } from "lucide-react";
  ...
  async function handleUpload() {
    ...
    try {
      await api("/api/v1/certificates", {
        method: "POST",
        body: JSON.stringify({ name: formName, cert_pem: formCert, key_pem: formKey }),
      });
      toast.success("证书已上传");
      setUploadOpen(false);
  ...
  return (
    <div className="space-y-6">
      <PageIntro
        ...
        actions={
          <Button className="gap-2 rounded-2xl bg-white text-slate-950 hover:bg-slate-100" onClick={() => { setUploadOpen(true); setFormName(""); setFormCert(""); setFormKey(""); }}>
                  ...
                  <TableRow key={cert.id} className="hover:bg-slate-50">
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        <FileKey2 className="h-4 w-4 text-cyan-600" />
                    ...
                    <TableCell>
                      <div className="flex items-center justify-end gap-1">
                        <Button variant="ghost" size="icon-sm" className="rounded-xl" onClick={() => setDetailCert(cert)}>
                          <Eye className="h-4 w-4" />
                        </Button>
                        <Button variant="ghost" size="icon-sm" className="rounded-xl text-rose-600 hover:bg-rose-50 hover:text-rose-700" onClick={() => setDeleteTarget(cert)}>
      ...
      <Dialog open={uploadOpen} onOpenChange={setUploadOpen}>
        <DialogContent className="max-w-lg rounded-[28px] max-h-[85vh] overflow-y-auto">
          <DialogHeader>
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              <Label>证书名称</Label>
              <Input value={formName} onChange={(e) => setFormName(e.target.value)} placeholder="例如：example.com" className="rounded-xl" />
            </div>
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <Label>证书内容 (PEM)</Label>
                <label className="cursor-pointer text-xs text-cyan-600 hover:text-cyan-700">
              ...
              <Textarea value={formCert} onChange={(e) => setFormCert(e.target.value)} rows={6} className="rounded-xl font-mono text-xs" placeholder="-----BEGIN CERTIFICATE-----" />
            </div>
            <div className="space-y-2">
              <div className="flex items-center justify-between">
              ...
              <Textarea value={formKey} onChange={(e) => setFormKey(e.target.value)} rows={6} className="rounded-xl font-mono text-xs" placeholder="-----BEGIN PRIVATE KEY-----" />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setUploadOpen(false)}>取消</Button>
            <Button onClick={handleUpload} disabled={uploading}>{uploading ? "上传中..." : "上传"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!detailCert} onOpenChange={(open) => !open && setDetailCert(null)}>
        <DialogContent className="max-w-xl rounded-[28px] max-h-[85vh] overflow-y-auto">
          <DialogHeader>
          ...
          {detailCert && (
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <Label>私钥 PEM</Label>
                <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 max-h-[120px] overflow-y-auto">
