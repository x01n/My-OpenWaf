...
export interface RuleBatchUpdateResponse {
  updated: number
  total: number
}

export interface OWASPRuleUpdateResponse {
  rule_id: string
...
export async function getOWASPRules(
  category?: string
): Promise<OWASPRulesResponse> {
...
): Promise<OWASPRuleUpdateResponse> {
  return api<OWASPRuleUpdateResponse>(`/api/v1/owasp-rules/${id}/update`, {
    method: "POST",
...
): Promise<RuleBatchUpdateResponse> {
  return api<RuleBatchUpdateResponse>("/api/v1/owasp-rules/batch", {
    method: "POST",
...
export async function getSensitivityConfig(
  protectionId: number | string
): Promise<SensitivityConfig> {
  return api<SensitivityConfig>(
    `/api/v1/protection/${protectionId}/sensitivity`
  )
}
...
export async function previewErrorPage(
  html: string,
  statusCode?: number
...
export async function batchToggleCVERules(
  ids: number[],
  enabled: boolean
): Promise<RuleBatchUpdateResponse> {
  return api<RuleBatchUpdateResponse>("/api/v1/cve-rules/batch", {
    method: "POST",
