    ...
    items: [
      {
        href: "/protection/",
        label: "攻击防护",
        description: "管理 OWASP、CVE 等检测规则与默认防护能力。",
        icon: Shield,
        group: "防护策略",
        children: [
          {
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        href: "/fingerprints/",
        label: "客户端指纹",
        description: "按 JA3 Hash / JA4 聚合 TLS 客户端指纹。",
        icon: Fingerprint,
        group: "日志审计",
      },
    ],
...
export function normalizeConsolePath(pathname: string | null | undefined) {
  ...
  return pathWithoutSearch.endsWith("/")
    ? pathWithoutSearch
    : `${pathWithoutSearch}/`
}

export function isConsoleNavPathActive(
  pathname: string | null | undefined,
  item: ConsoleNavItem
) {
  const currentPath = normalizeConsolePath(pathname)
  const itemPath = normalizeConsolePath(item.href)

  if (currentPath === itemPath) return true
  return item.exact === false && currentPath.startsWith(itemPath)
}

export function getNavMeta(pathname: string | null | undefined) {
  const item = consoleNavItems.find((entry) =>
    isConsoleNavPathActive(pathname, entry)
  )
...
export const wafActionOptions: WAFActionMeta[] = [
  ...
  },
  {
    ...
    defaultStatus: "422",
    className: "border-secondary bg-secondary text-secondary-foreground",
    description: "PoW、JavaScript/WASM 与环境一致性检查。",
  },
  {
...
export const nonRedirectWAFActionOptions = terminalWAFActionOptions.filter(
  (item) => item.value !== "redirect"
)
export const antiReplayWAFActionOptions = wafActionOptions.filter((item) =>
  [
    "challenge",
    "captcha_challenge",
    "shield_challenge",
