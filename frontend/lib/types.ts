...
/** JavaScript 边缘插件，字段严格对齐 store.JSPlugin。 */
...
/** 当前 JS 运行时不可用时返回的响应。 */
export interface JSPluginRuntimeUnavailableResponse {
  error: string;
  items?: unknown[];
  total?: number;
}

/** JS 插件统计响应；当前后端仅返回运行时不可用信息。 */
export type JSPluginStatsResponse = JSPluginRuntimeUnavailableResponse;

/** JS 插件 dry-run 当前无可用运行时，因此没有可推断的结果字段。 */
export type JSPluginDryRunResponse = JSPluginRuntimeUnavailableResponse;
