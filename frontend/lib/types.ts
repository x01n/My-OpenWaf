...
export type TLSConfigUpdate = Partial<Omit<TLSConfig, "has_explicit_default_alpn">>
...
export interface LuaPlugin {
  ...
  /** 同阶段内的执行顺序，数值小者先执行 */
  priority: number
  /** 作用域站点 ID；null/undefined 表示全局，非空时仅对同 ID 请求执行 */
  site_id?: number | null
  /** 执行超时（毫秒），0 表示用后端默认值 */
  timeout_ms: number
  /** 运维备注 */
 ...
 * dry-run 直接执行脚本，不模拟线上 Engine 的 site_id 过滤或 KV 前后置 fail-open。
 */
export interface LuaDryRunRequestView {
  ...
  host?: string
  user_agent?: string
  /** 仅构造脚本可见的 ctx.site_id；dry-run 不执行 Engine 的脚本站点作用域过滤 */
  site_id?: number
  content_type?: string
  body?: string
...
export interface LuaDryRunRequest {
  ...
  /** 0 或省略表示用后端默认超时 */
  timeout_ms?: number
  /** 连续执行同一个样例请求的次数，用于复现依赖 ctx.kv 的窗口类策略 */
  iterations?: number
...
export interface LuaDecision {
  /** 空串表示脚本未判定，请求继续走后续阶段；字符串上限 16 KiB */
  Action: string
  /** 字符串上限 16 KiB */
  Message: string
  /** 仅允许 / 开头的相对路径或 http/https 绝对 URL，字符串上限 16 KiB */
  RedirectTo: string
  /** 0 表示用默认响应码；后端只接受 100–599 */
  StatusCode: number
  /**
   * 受控响应头：最多 256 项，名称必须合法且不超过 16 KiB，每值最多 4 KiB；非法、敏感、逐跳及
   * Content-Length/Location 等宿主管理字段会被后端拒绝。
   */
  SetHeaders: Record<string, string> | null
  /**
   * 可选响应体，最多 16 KiB 并按 UTF-8 边界截断。
   * 只由普通 Lua 终止响应消费，不覆盖 challenge/redirect/drop/上游响应。
   */
  ResponseBody: string
  /** 最多 32 项；已进入 action.Result，但当前尚未持久化到日志 */
  Tags: string[] | null
}
...
export interface LuaDryRunRunResult {
  /** 非空表示未通过编译，此时其余字段无意义 */
...
export interface LuaDryRunResult extends LuaDryRunRunResult {
  /** 请求的连续执行次数，后端会限制在 1 到 100 */
  iterations: number
  /** 当前 dry-run 是否拿到了可用的 ctx.kv 后端 */
  kv_available: boolean
  /** 每次执行的完整轨迹；命中判定或运行错误后停止 */
  runs?: LuaDryRunRunResult[]
}
