...
- 文档：docs/配置管理系统/迁移管理机制.md、docs/数据存储层/数据迁移管理.md
...
MIGV2["历史迁移脚本<br/>internal/store/migrations/v2_single_site.go"]
end
subgraph "运行时层"
RT["运行时与快照<br/>internal/core/runtime.go"]
end
subgraph "数据库层"
DB["GORM 连接<br/>internal/core/database/gorm.go"]
CR["配置修订表<br/>config_revisions"]
end
APP --> MIG
APP --> SEED
MIG --> MIGV2
...
participant Rev as "配置修订<br/>config_revisions"
...
- 执行顺序
  ...
  - 结构迁移覆盖证书、策略、规则、站点、系统设置、管理员 API Key、配置修订、管理员账户、刷新令牌、安全事件、IP 黑白名单、令牌黑名单、登录尝试、活动会话、丢弃事件、机器人评分日志、CVE 规则记录、CVE 同步日志、应用路由规则、记录资源等。
- 版本管理
...
Server --> Seed["seed.go"]
Migrate --> MigrateV2["migrations/v2_single_site.go"]
Migrate --> DB["gorm.go"]
Seed --> DB
Server --> Runtime["runtime.go"]
Runtime --> DB
Runtime --> Rev["config_revisions 表"]
```
