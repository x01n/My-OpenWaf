...
- [docs/配置管理系统/热重载系统.md:30-77](file://docs/配置管理系统/热重载系统.md#L30-L77)
...
### 速率限制器与 IP 黑白名单刷新
- 速率限制器
  - 支持本地与 Redis 两种后端，运行时可动态调整窗口大小、最大请求数与启用状态。
  - 在热重载时根据快照中的保护配置重新配置，确保策略一致性。
- IP 黑白名单
  - 从数据库加载启用的条目，支持黑名单与白名单，动态应用到 IP 信誉模块。
  - 热重载时重新加载列表，同时可配置自动封禁策略（阈值、窗口、持续时间、动作）。

章节来源
- [internal/waf/ratelimit/ratelimit.go:1-127](file://internal/waf/ratelimit/ratelimit.go#L1-L127)
- [internal/app/server.go:420-441](file://internal/app/server.go#L420-L441)

### 自动封禁策略协调
- 配置来源
  - 保护配置中的自动封禁开关、阈值、窗口、持续时间与动作类型。
- 执行机制
  - 通过 IP 信誉模块实现，错误率超过阈值时触发封禁。
  - 封禁动作可配置为拦截或阻断，影响后续请求处理。

章节来源
- [internal/app/server.go:208-234](file://internal/app/server.go#L208-L234)

### 数据平面监听器的协调重启
- 监听器指纹
  - 基于绑定地址与站点配置生成短哈希指纹，任何配置变化都会导致指纹变化。
- 协调逻辑
  - 比较现有监听器与期望配置，移除指纹变化或缺失的实例，启动新的监听器。
  - 仅影响受影响的站点监听器，避免全站重启。

章节来源
- [internal/app/server.go:253-311](file://internal/app/server.go#L253-L311)
- [internal/app/server.go:605-636](file://internal/app/server.go#L605-L636)
...
章节来源
- [docs/配置管理系统/热重载系统.md:306-317](file://docs/配置管理系统/热重载系统.md#L306-L317)
- [internal/app/server.go:268-278](file://internal/app/server.go#L268-L278)

### API 调用示例与配置变更流程
- 管理 API 路由注册
  - Admin 服务器注册健康检查、就绪检查、状态查询与指标端点。
  - 注册路由时注入重载函数，供管理员操作使用。
- 重载流程
  - 递增配置修订号 → 重新构建快照 → 应用保护配置 → 加载 IP 列表 → 协调监听器 → Redis 广播。

章节来源
- [internal/app/server.go:351-372](file://internal/app/server.go#L351-L372)
- [internal/app/server.go:313-334](file://internal/app/server.go#L313-L334)