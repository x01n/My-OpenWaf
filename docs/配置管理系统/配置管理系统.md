...
### 核心运行时
- [配置快照机制](./配置快照机制.md) — 不可变快照、原子指针切换与版本管理，实现零锁争用的高并发配置读取。
- [热重载系统](./热重载系统.md) — 配置变更的平滑切换与零停机重载，涵盖快照重建、监听器协调与分布式通知。

### 验证与同步
- [配置验证系统](./配置验证系统.md) — 启动前与运行时的多层校验，覆盖数据格式、业务规则与规则模式验证。
- [分布式同步机制](./分布式同步机制.md) — 基于 Redis Pub/Sub 的跨节点配置重载广播，保证最终一致性。

### 数据演进
- [迁移管理机制](./迁移管理机制.md) — 数据库历史迁移、结构迁移、版本管理与种子数据，确保配置安全演进与向后兼容。
