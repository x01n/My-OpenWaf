...
- **数据库基础配置**
  - [GORM 配置与使用](./GORM 配置与使用.md) — 详解 GORM 初始化、连接池、日志、事务、查询优化与性能调优实践。
  - [多数据库支持](./多数据库支持.md) — 覆盖 SQLite、MySQL、PostgreSQL 驱动选择、DSN 解析、连接池差异与方言兼容性处理。
- **数据模型与访问**
  - [数据库模型设计](./数据库模型设计.md) — 阐述实体关系、字段规范、索引策略、软删除机制及各领域模型（站点、策略、认证、事件等）的 ER 关系。
  - [数据访问层设计](./数据访问层设计.md) — 介绍仓储模式（Repository Pattern）、Repos 聚合、依赖注入、CRUD 最佳实践及安全相关仓储详解。
- **缓存与分布式**
  - [Redis 集成](./Redis 集成.md) — 覆盖 Redis 连接配置、分布式键值缓存、响应缓存、发布/订阅、分布式限流与缓存策略。
- **数据迁移与演进**
  - [数据迁移管理](./数据迁移管理.md) — 阐述迁移文件组织、版本管理、执行顺序、配置升级、种子数据管理与回滚策略。
