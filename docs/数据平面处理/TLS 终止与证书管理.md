...
My-OpenWaf 是一个基于 Go 语言开发的 Web 应用防火墙，提供了完整的 TLS 终止与证书管理功能。该系统采用云雀(Hertz)框架作为 HTTP 服务器，实现了高性能的 TLS 终止、多域名证书支持和智能证书管理。

本文档深入解析了系统的 TLS 终止机制、证书管理流程、SNI 处理策略以及相关的安全配置选项，为开发者和运维人员提供全面的技术指导。
...
subgraph "证书管理"
E[internal/admin/handler_certificate.go]
F[internal/store/repository/certificate.go]
G[internal/store/models.go]
end
subgraph "前端界面"
...
**章节来源**
- [handler_certificate.go:45-109](file://internal/admin/handler_certificate.go#L45-L109)
...
- [handler_certificate.go:3-13](file://internal/admin/handler_certificate.go#L3-L13)
