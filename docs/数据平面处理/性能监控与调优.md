...
C --> F["事件写入(异步批处理)<br/>internal/observability/unified_writer.go"]
C --> G["响应缓存<br/>internal/cache/response_cache.go"]
C --> H["限流器<br/>internal/waf/ratelimit/ratelimit.go"]
...
### 连接复用与上游传输
- HTTPTransport 返回一个连接池化的传输层，适用于反向代理到站点上游
- 关键参数
  - MaxIdleConns：全局空闲连接数上限
  - MaxIdleConnsPerHost：每主机空闲连接数上限
  - IdleConnTimeout：空闲连接超时
  - ForceAttemptHTTP2：强制尝试 HTTP/2
...
### 对象池优化与资源管理
- 对象池优化
  - RequestCtx 使用 sync.Pool 复用，减少 GC 压力
  - 响应缓存采用 64 分片哈希锁，降低锁竞争
- 连接复用机制
  - HTTPTransport 配置合理的空闲连接上限与超时，提升连接复用效率
- 资源管理策略
  - 统一的 Runtime 管理数据库、Redis、缓存与快照，确保资源生命周期一致
  - 通过分片缓存与连接池参数化，平衡内存占用与吞吐性能
