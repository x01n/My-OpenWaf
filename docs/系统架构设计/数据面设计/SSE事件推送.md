...
本文档系统性阐述 OpenWAF 中 Server-Sent Events（SSE）事件推送的实现与使用方式，覆盖以下主题：
...
classDiagram
class EventWriter {
-private repo : SecurityEventRepo
-private ch : chan SecurityEvent
-private log : Logger
-private stopCh : chan struct
-private wg : WaitGroup
-private batchSize : int
-private flushInterval : time.Duration
+Record(ev)
+Close()
-loop()
-flush(buf)
