...
My-OpenWaf 的 WebSocket 处理功能是一个完整的实时通信安全网关解决方案。该系统基于 CloudWeGo Hertz 框架构建，提供了标准 WebSocket 协议的完整支持，包括协议升级处理、双向数据传输、实时威胁检测和动态阻断能力。

系统的核心优势在于其模块化架构设计，将 WebSocket 处理与 WAF 引擎深度集成，实现了从协议层面到应用层面的全方位安全保障。通过智能的连接池管理和性能优化策略，系统能够在保证安全性的前提下提供高性能的实时通信服务。

## 项目结构

My-OpenWaf 采用分层模块化架构，WebSocket 功能主要分布在以下关键模块中：
...
WebSocket 协议升级检测器负责识别客户端发起的 WebSocket 连接请求，确保只有符合标准的 WebSocket 握手请求才能被处理。
...
WebSocket 连接隧道转发器实现了完整的双向数据传输机制，通过 goroutine 实现高效的数据复制和传输。
...
- [drop.go:61-83](file://internal/waf/drop/drop.go#L61-L83)
