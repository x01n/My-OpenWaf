...
- [docs/WAF 引擎系统/规则管道设计/处理阶段详解.md](file://docs/WAF 引擎系统/规则管道设计/处理阶段详解.md)
</cite>
...
## 简介
本文件系统性梳理 OpenWAF 的“动作集成”设计与实现，围绕以下目标展开：
- 动作类型定义与标准化（含兼容性映射）
- 执行器接口与配置管理
- 内置动作的实现机制（阻断、允许、观察、丢弃、维护模式、阻断页渲染）
...
- [internal/waf/pages/block.go:16-110](file://internal/waf/pages/block.go#L16-L110)
...
章节来源
- [internal/waf/drop/drop.go:19-123](file://internal/waf/drop/drop.go#L19-L123)
- [internal/admin/protect/drop.go:24-121](file://internal/admin/protect/drop.go#L24-L121)
...
### 动作链组合与嵌套机制
- 阶段顺序
  - IP 信誉 → ACL → 机器人检测 → 请求限速 → OWASP → CVE → 签名 → 自定义。
- 优先级与短路
  - Pipeline 逐阶段执行，遇到丢弃（最高优先级）立即短路；拦截亦为终端。
  - 观察命中仅收集日志，不短路。
- 条件分支与复合规则
...
### 动作测试与验证
- 动作效果验证
  - 使用规则测试接口对 DSL 进行干跑，验证匹配与动作类型。
  - 前端规则构建器提供 DSL 预览与简单校验。
- 性能测试
  - 对复杂复合规则与大体量请求进行吞吐与延迟测试，关注正则扫描范围与 JSON/表单解析开销。
- 安全审计
  - 通过 Drop 执行器统计来源分布，结合阻断页日志进行溯源。
  - 对观察命中进行聚合统计，识别潜在攻击趋势。
...
- [internal/waf/drop/drop.go:59-83](file://internal/waf/drop/drop.go#L59-L83)
- [internal/waf/pages/block.go:68-110](file://internal/waf/pages/block.go#L68-L110)
- [internal/admin/protect/drop.go:24-121](file://internal/admin/protect/drop.go#L24-L121)
