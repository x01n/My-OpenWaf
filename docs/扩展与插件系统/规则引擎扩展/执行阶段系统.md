...
- [docs/扩展与插件/规则引擎扩展/执行阶段系统.md](file://docs/扩展与插件/规则引擎扩展/执行阶段系统.md)
...
- 阶段划分：预过滤（IP信誉白名单短路）、请求处理（ACL/签名/自定义）、响应处理（速率限制、OWASP/CVE检测、机器人检测）等阶段的职责与顺序。
...
**图表来源**
- [docs/扩展与插件/规则引擎扩展/执行阶段系统.md:49-99](file://docs/扩展与插件/规则引擎扩展/执行阶段系统.md#L49-L99)

**章节来源**
- [docs/扩展与插件/规则引擎扩展/执行阶段系统.md:40-48](file://docs/扩展与插件/规则引擎扩展/执行阶段系统.md#L40-L48)
...
**图表来源**
- [docs/扩展与插件/规则引擎扩展/执行阶段系统.md:125-158](file://docs/扩展与插件/规则引擎扩展/执行阶段系统.md#L125-L158)
...
**图表来源**
- [docs/扩展与插件/规则引擎扩展/执行阶段系统.md:180-199](file://docs/扩展与插件/规则引擎扩展/执行阶段系统.md#L180-L199)

**章节来源**
- [docs/扩展与插件/规则引擎扩展/执行阶段系统.md:167-179](file://docs/扩展与插件/规则引擎扩展/执行阶段系统.md#L167-L179)
...
- 调度策略：
  ...
  - 优先级：Drop/Intercept 立即短路；Allow 跳过后续阶段。
  - 顺序：IP信誉 → ACL → 机器人 → 速率限制 → OWASP → CVE → 签名 → 自定义。
...
- [docs/扩展与插件/规则引擎扩展/执行阶段系统.md:293-299](file://docs/扩展与插件/规则引擎扩展/执行阶段系统.md#L293-L299)
...
**章节来源**
- [docs/扩展与插件/规则引擎扩展/执行阶段系统.md:305-314](file://docs/扩展与插件/规则引擎扩展/执行阶段系统.md#L305-L314)
...
**图表来源**
- [docs/扩展与插件/规则引擎扩展/执行阶段系统.md:329-344](file://docs/扩展与插件/规则引擎扩展/执行阶段系统.md#L329-L344)

**章节来源**
- [docs/扩展与插件/规则引擎扩展/执行阶段系统.md:319-328](file://docs/扩展与插件/规则引擎扩展/执行阶段系统.md#L319-L328)

## 性能考虑
- 对象池：RequestCtx 使用 sync.Pool 复用，显著降低 GC 压力，建议在高频路径保持复用。
- 短路策略：Drop/Intercept 立即短路，避免无效计算；Allow 跳过后续阶段，减少规则匹配成本。
...
**章节来源**
- [docs/扩展与插件/规则引擎扩展/执行阶段系统.md:370-380](file://docs/扩展与插件/规则引擎扩展/执行阶段系统.md#L370-L380)
...
- 快速参考：阶段顺序与短路优先级
  - 顺序：IP信誉 → ACL → 机器人 → 速率限制 → OWASP → CVE → 签名 → 自定义
  - 优先级：Drop > Intercept > Observe > Allow（Allow 跳过后续阶段）