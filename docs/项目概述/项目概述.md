...
IPREP["internal/waf/iprep.go<br/>IP 黑白名单/自动封禁"]
end
subgraph "存储与缓存"
...
- 引擎（Engine）
  - 将请求上下文送入多阶段流水线，依次执行 IP 名单、ACL、机器人检测、速率限制、OWASP 规则、签名与自定义规则。
  - 支持评估模式（仅规则链）与维护模式短路。
- 流水线（Pipeline）
  - 定义统一的阶段接口，顺序执行，遇到终端动作立即短路，观察命中用于日志记录。
- 速率限制（RateLimiter）
...
- [internal/waf/iprep.go:44-180](file://internal/waf/iprep.go#L44-L180)
- [internal/admin/router.go:36-137](file://internal/admin/router.go#L36-L137)
...
- 阶段顺序：
  1) IP 名单：白名单短路放行，黑名单直接拦截。
  2) ACL：基于站点规则的访问控制。
  3) 机器人检测：在速率限制前尽早阻断恶意工具。
  4) 请求速率限制：按窗口与阈值控制。
  5) OWASP 规则集：可选启用的标准规则族。
  6) 签名与自定义规则：复合规则与用户自定义规则。
- 终止条件：遇到拦截动作立即返回；观察命中用于日志记录。
