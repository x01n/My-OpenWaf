...
## 引言
本文件面向中间件开发与运维工程师，系统性阐述本项目的中间件体系：包括中间件接口与执行顺序、上下文传递、注册与优先级、依赖管理；以及中间件池化机制（请求上下文对象池）；内置中间件类型（认证、日志、安全头）；自定义中间件开发规范；以及配套的测试、性能分析与故障排查方法。目标是帮助读者快速理解并扩展中间件能力，构建稳定高效的防护与控制平面。
...
B --> D["internal/admin/handler_auth.go<br/>鉴权处理器"]
...
G --> J["internal/waf/ratelimit.go<br/>速率限制"]
G --> K["internal/waf/bot.go<br/>机器人检测"]
G --> L["internal/waf/iprep.go<br/>IP信誉"]
M["internal/waf/block.go<br/>拦截/维护页"] --> B
...
- 阶段顺序
  - IP信誉（白名单短路、黑名单阻断）、ACL、机器人检测（可选）、请求速率限制（可选）、OWASP、CVE、签名与自定义规则。
- 结果封装
...
Router --> HAuth["admin/handler_auth.go"]
...
- 认证问题
  ...
  - 参考路径：[internal/admin/middleware.go:29-71](file://internal/admin/middleware.go#L29-L71)，[internal/admin/handler_auth.go:125-221](file://internal/admin/handler_auth.go#L125-L221)
- 会话与权限
...
- 速率限制
  ...
  - 参考路径：[internal/waf/ratelimit.go:40-92](file://internal/waf/ratelimit.go#L40-L92)
- 机器人检测
  - 预筛选命中导致进入深度评分；检查 UA、GeoIP 与 IP信誉配置。
  - 参考路径：[internal/waf/bot.go:126-161](file://internal/waf/bot.go#L126-L161)
- 拦截/维护页
  - 检查站点模板与状态码配置；若模板为空则回退到嵌入式页面。
  - 参考路径：[internal/waf/block.go:16-66](file://internal/waf/block.go#L16-L66)
...
- [internal/admin/handler_auth.go:125-221](file://internal/admin/handler_auth.go#L125-L221)
