...
## 简介
本文件面向规则编译器扩展的开发者与维护者，系统性阐述规则编译器的核心架构与实现细节，包括规则解析、编译过程、执行器构建机制、排序逻辑以及扩展新规则类型的完整流程。文档同时提供最佳实践与性能优化建议，并通过图示帮助读者快速理解从规则DSL到运行时匹配器的全链路。
...
- [handler_rule_validate.go:1-201](file://internal/admin/handler_rule_validate.go#L1-L201)
