...
- [internal/waf/blazetest_test.go](file://internal/waf/blazetest_test.go)
</cite>
...
- 执行阶段：按阶段顺序执行，包括 IP 白黑名单、ACL、Bot 检测、速率限制、OWASP、CVE、签名与自定义规则等。
...
### 规则执行阶段机制
- 阶段顺序：IP 白名单/黑名单 → ACL → Bot 检测 → 速率限制 → OWASP → CVE → 签名 → 自定义。
...
- Body：body_contains（占位，实际在请求上下文处理）。
- 复合条件：and/or/not，支持递归嵌套。
