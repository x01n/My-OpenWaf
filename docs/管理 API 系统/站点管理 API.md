...
- 基础信息
  ...
  - 上游主机（upstream_host）：可留空；支持域名、域名:端口或 Go template，用于指定发给上游的 Host 头。
- 监听配置
...
- TLS 配置
  ...
  - 证书 ID（cert_id）：关联证书表。
  - 最小/最大 TLS 版本（min_tls_version, max_tls_version）：空值表示继承全局 TLS 默认配置；当前全局默认最小/最大版本为 TLS10/TLS13。
  - 密码套件（cipher_suites）：可选。
  - ALPN 协议（alpn）：空值表示继承全局 ALPN 默认配置；当前全局默认随 HTTP/2 与 HTTP/3 开关派生，默认启用时为 "h2,h3,http/1.1"。
- 保护配置
  - 策略 ID（policy_id）：可选。
...
- 创建站点
  - 方法：POST
  - 路径：/api/v1/sites
  - 请求体：站点对象（含 host, upstream_urls, upstream_host, bind 等）
  - 响应：创建后的站点对象
- 更新站点
  - 方法：POST
