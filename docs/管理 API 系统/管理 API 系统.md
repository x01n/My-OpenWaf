...
- [docs/管理 API 系统/REST API 设计规范/REST API 设计规范.md](file://docs/管理 API 系统/REST API 设计规范/REST API 设计规范.md)
- [docs/管理 API 系统/REST API 设计规范/认证授权机制.md](file://docs/管理 API 系统/REST API 设计规范/认证授权机制.md)
- [docs/管理 API 系统/REST API 设计规范/请求响应格式规范.md](file://docs/管理 API 系统/REST API 设计规范/请求响应格式规范.md)
- [docs/管理 API 系统/REST API 设计规范/路由设计规范.md](file://docs/管理 API 系统/REST API 设计规范/路由设计规范.md)
- [docs/管理 API 系统/REST API 设计规范/错误处理机制.md](file://docs/管理 API 系统/REST API 设计规范/错误处理机制.md)
- [docs/管理 API 系统/站点管理 API.md](file://docs/管理 API 系统/站点管理 API.md)
- [docs/管理 API 系统/策略管理 API.md](file://docs/管理 API 系统/策略管理 API.md)
- [docs/管理 API 系统/系统设置 API.md](file://docs/管理 API 系统/系统设置 API.md)
- [docs/管理 API 系统/证书管理 API.md](file://docs/管理 API 系统/证书管理 API.md)
</cite>

## 模块架构概述

管理 API 系统是 My-OpenWaf 控制面（Admin Control Plane）的核心，基于 Hertz 框架提供版本化的 REST API（`/api/v1`）。系统采用分层架构：

- **路由与中间件层**：统一注册端点，挂载安全头、访问日志、JWT/API Key 认证、RBAC 角色权限校验
- **处理器层**：按模块划分（站点、规则、策略、证书、IP 列表、安全事件、系统设置等），负责业务逻辑与输入校验
- **仓储层**：基于 GORM 的仓库模式，提供 CRUD、分页、过滤、聚合统计
- **快照与热重载层**：配置变更后触发快照重建，原子替换运行时配置并协调数据面监听器热更新
- **前端 SDK 层**：统一的 `api.ts` 封装，自动处理令牌刷新、错误码与分页

子页面按主题分为四大类：**基础规范**（API 设计、认证、路由、错误处理）、**核心资源管理**（站点、规则、策略）、**安全与防护配置**（证书、IP 黑白名单、Bot、CVE、阻断策略）、**监控与运维**（安全事件、仪表板、系统设置）。

## 子页面分类索引

### 基础规范

> 定义管理 API 的通用设计原则、端点规范、认证授权模型、路由组织与错误处理机制。

- [REST API 设计规范](./REST API 设计规范/REST API 设计规范.md) — 管理 API 的总设计规范，涵盖版本控制、资源命名、HTTP 方法、安全策略与前端 SDK 指南
- [API 端点参考](./REST API 设计规范/API 端点参考.md) — 完整端点速查手册，汇总所有资源的 URL、方法、权限与响应格式
- [认证授权机制](./REST API 设计规范/认证授权机制.md) — JWT 短期访问令牌、刷新令牌、API Key、RBAC 角色与暴力破解防护
- [请求响应格式规范](./REST API 设计规范/请求响应格式规范.md) — 统一 JSON 响应结构、分页机制、排序规则与错误码对照
- [路由设计规范](./REST API 设计规范/路由设计规范.md) — 版本前缀、资源路径设计、中间件链、SPA 回退与静态文件处理
- [错误处理机制](./REST API 设计规范/错误处理机制.md) — 错误类型定义、统一响应格式、日志记录、降级与故障恢复策略

### 核心资源管理

> 覆盖 WAF 核心资源的增删改查与生命周期管理，包括站点、规则与策略三大支柱。

- [站点管理 API](./站点管理 API.md) — 站点生命周期（创建、配置、启停、删除）、域名绑定、TLS、上游配置与状态监控
- [规则管理 API](./规则管理 API/规则管理 API.md) — 规则系统总览：DSL 语法、编译器、匹配引擎、执行阶段与前端可视化构建器
- [规则 CRUD 操作](./规则管理 API/规则 CRUD 操作.md) — 规则的创建、读取、更新、删除、分页查询与详细 API 规范
- [规则验证与测试](./规则管理 API/规则验证与测试.md) — 规则语法验证、即时匹配测试、复合规则 JSON 校验与调试方法
- [规则导入导出](./规则管理 API/规则导入导出.md) — 规则批量导入导出、模板系统、跨环境迁移与版本兼容性
- [策略管理 API](./策略管理 API.md) — 策略分组管理、策略与规则的继承关系、快照构建与阶段化执行

### 安全与防护配置

> 提供安全加固相关的 API，包括证书、IP 访问控制、Bot 检测、CVE 规则与连接丢弃策略。

- [证书管理 API](./证书管理 API.md) — TLS 证书的 CRUD、PEM 配对校验、证书链管理与监听器热更新
- [IP 黑白名单管理 API](./IP 黑白名单管理 API.md) — IP/CIDR 访问控制列表、白名单优先机制、自动封禁与 IP 信誉集成
- [阻断策略 API](./阻断策略 API.md) — TCP 连接丢弃策略配置、24 小时统计、丢弃事件查询与异步写入
- [机器人保护 API](./机器人保护 API.md) — Bot 检测全局配置、评分阈值、高风险国家/ASN、评分日志查询与指纹统计
- [CVE 规则管理 API](./CVE 规则管理 API.md) — CVE 漏洞规则 CRUD、NVD/GitHub 自动同步、批量操作与规则统计

### 监控与运维

> 面向运营人员的安全事件分析、实时监控与系统配置管理接口。

- [安全事件 API](./安全事件 API.md) — 安全事件查询（多维过滤、分页）、统计分析（Top IP/Path/Rule）、时间线与 CSV 导出
- [仪表板 API](./仪表板 API.md) — 实时 QPS、请求总量、状态码分布、WAF 拦截统计与系统健康摘要
- [系统设置 API](./系统设置 API.md) — 全局键值配置、防护设置（限流/OWASP/Bot/CC）、热重载与跨节点 Redis 同步

## 简介
本文件为 My-OpenWaf 后台管理 API 的完整端点参考与设计规范，覆盖站点管理、规则管理、策略配置、证书管理、IP 黑白名单、系统设置、安全事件、防护设置、仪表盘统计等模块。内容包括：
