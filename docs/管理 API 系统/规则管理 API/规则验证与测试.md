...
subgraph "后端"
R["路由注册<br/>router.go"]
VR["规则验证处理器<br/>validate.go"]
TR["规则测试处理器<br/>rule.go"]
...
章节来源
- [rule.go:104-156](file://internal/admin/rule/rule.go#L104-L156)
...
```mermaid
graph LR
ROUTER["路由注册<br/>router.go"] --> VALIDATE["验证处理器<br/>validate.go"]
ROUTER --> TEST["测试处理器<br/>rule.go"]
...
### 规则验证 API（POST /api/v1/rules/validate）
- 请求体
  - pattern: string（必填）- 规则 DSL 或复合 JSON
- 响应
  - valid: boolean - 验证是否通过
  - message: string - 验证消息
  - kind: string - 规则类型（如 block_ip, block_path_regex 等）
  - arg: string - 规则参数
  - errors: string[] - 错误列表（验证失败时）

章节来源
- [validate.go:13-23](file://internal/admin/rule/validate.go#L13-L23)
- [validate.go:32-98](file://internal/admin/rule/validate.go#L32-L98)

### 规则测试 API（POST /api/v1/rules/test）
- 请求体
  - pattern: string（必填）- 规则 DSL 或复合 JSON
  - client_ip: string - 客户端 IP（可选）
  - path: string - 请求路径（必填）
  - query: string - 查询字符串（可选）
  - headers: map[string]string - 请求头映射（可选）
- 响应
...
章节来源
- [rule.go:140-147](file://internal/admin/rule/rule.go#L140-L147)
- [rule.go:151-191](file://internal/admin/rule/rule.go#L151-L191)

### 前端规则构建器集成
- 本地测试功能：支持简单规则的即时匹配测试，适用于基本的 IP、路径、方法、头部、查询、Body 匹配场景。
- 后端验证集成：通过 /api/v1/rules/validate 实现规则语法验证，确保规则可被后端正确解析与编译。
- 复合规则限制：前端本地测试不支持复合 JSON 规则的精确匹配，需依赖后端验证接口。

章节来源
- [rule-builder.tsx:235-301](file://frontend/components/rule-builder.tsx#L235-L301)

### 规则质量评估指标与测试报告
- 语法正确率：通过 /api/v1/rules/validate 的成功率
- 匹配准确率：通过 /api/v1/rules/test 的命中准确性
- 性能指标：正则编译缓存命中率、匹配耗时分布
- 建议的测试报告字段：
  - 规则 ID/名称
  - 验证状态与错误详情
  - 测试样例集与命中结果
...
章节来源
- [规则验证与测试.md:320-344](file://docs/管理 API 系统/规则管理 API/规则验证与测试.md#L320-L344)