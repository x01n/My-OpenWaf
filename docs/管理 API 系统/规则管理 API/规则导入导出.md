...
- 功能要点
  ...
  - 对每个规则执行批量创建操作，ID 设为 0 以强制生成新 ID
  - 统计成功导入数量并返回结果
- 错误处理
  - 请求体解析失败返回 400
  - 未提供规则数组返回 400
...
Valid --> |否| Loop["遍历每条规则"]
Loop --> ValidateAction["验证并规范化 action"]
...
- 导入性能
  - 导入采用批量创建，适合批量导入；如需更高吞吐，可在前端分批上传或后端引入事务批量插入
- 规则编译
    ...
    "rules": [
      {
        ...
        "phase": "acl|rate_limit|owasp_default|signature|custom",
        "pattern": "string",
        "action": "allow|intercept|observe|drop|block|log_only",
        "priority": 100,
        "enabled": true,
        "status_code": 0,
        "redirect_to": "string"
