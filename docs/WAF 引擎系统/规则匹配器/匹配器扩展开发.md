...
- [match_test.go](file://internal/appresource/match_test.go)
...
## 简介
本指南面向匹配器扩展开发者，系统性阐述如何实现自定义 Matcher 接口、扩展 buildMatcher 函数、注册新匹配器、编写测试与调试方法，以及匹配器在规则编译与执行过程中的集成方式。文档基于仓库内的规则引擎子系统，涵盖接口设计、内置实现、复合匹配器、正则缓存与 JSON 条件解析、阶段化执行与性能优化策略。
...
- 测试文件：matcher_test.go、compiler_test.go、match_test.go 等
...
#### 1. 实现自定义 Matcher 接口
- 定义结构体：包含必要的状态与配置字段
- 实现 Match 方法：根据需求读取 MatchCtx 字段，返回布尔值
- 注意参数处理：对空输入、非法格式进行健壮性处理

#### 2. 扩展 buildMatcher 函数
- 在 switch 语句中添加新的 kind 分支
- 实现参数解析与校验逻辑
- 返回新匹配器实例，必要时进行错误处理

#### 3. 注册新匹配器
- 在 ParsePattern 中添加新的 DSL 前缀
- 在 buildMatcher 中为新 kind 实现匹配器构造逻辑
- 如需复合条件支持，在 parseCompoundJSON/buildCompound 中处理新 kind

#### 4. 编写测试用例
- 单元测试：覆盖正常匹配、不匹配、边界条件与错误输入
- 性能测试：验证正则缓存与不同匹配器的耗时对比
- 集成测试：验证规则编译与执行流程

#### 5. 性能优化建议
- 使用 cachedCompile 缓存正则表达式
- 避免昂贵的字符串操作与正则回溯
- 合理使用短路与优先级排序
- 控制头部扫描范围与数量
...
#### 示例一：简单的字符串匹配器
- 目标：实现一个基于请求路径包含特定字符串的匹配器
- 步骤：
  1. 定义结构体：包含要匹配的字符串
  2. 实现 Match 方法：检查 Path 是否包含目标字符串
  3. 在 buildMatcher 中添加分支处理
  4. 编写单元测试验证行为
...
- 步骤：
  1. 定义复合结构体：包含多个子匹配器
  2. 实现 Match 方法：按逻辑组合子匹配器的结果
  3. 在 parseCompoundJSON 中支持新 kind
  4. 编写复杂场景的测试用例

### 匹配器在规则编译和执行中的集成

#### 编译阶段集成
- ParsePattern：识别新的 DSL 前缀并提取参数
- buildMatcher：根据 kind 创建匹配器实例
- Compile：编译规则并按优先级排序

#### 执行阶段集成
- MatchCtx：从 pipeline.RequestCtx 转换而来
- 阶段化执行：ACL、签名、自定义阶段依次遍历规则
- 短路机制：Allow 动作立即短路，其他动作根据配置决定
