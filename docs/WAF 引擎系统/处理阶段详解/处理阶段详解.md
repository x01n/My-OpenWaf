# 处理阶段详解

<cite>
**本文档引用的文件**
- [internal/core/engine/engine.go](file://internal/core/engine/engine.go)
- [internal/core/pipeline/pipeline.go](file://internal/core/pipeline/pipeline.go)
- [internal/core/rules/phases.go](file://internal/core/rules/phases.go)
- [internal/waf/iprep.go](file://internal/waf/iprep.go)
- [internal/waf/bot.go](file://internal/waf/bot.go)
- [internal/waf/ratelimit.go](file://internal/waf/ratelimit.go)
- [internal/waf/owasp.go](file://internal/waf/owasp.go)
- [internal/waf/geoip.go](file://internal/waf/geoip.go)
- [internal/waf/eval.go](file://internal/waf/eval.go)
- [internal/waf/semantic.go](file://internal/waf/semantic.go)
- [internal/waf/fingerprint.go](file://internal/waf/fingerprint.go)
- [internal/waf/fingerprint_db.go](file://internal/waf/fingerprint_db.go)
- [internal/waf/cve_detector.go](file://internal/waf/cve_detector.go)
- [internal/waf/cve_general.go](file://internal/waf/cve_general.go)
- [internal/waf/cve_java.go](file://internal/waf/cve_java.go)
- [internal/waf/cve_node.go](file://internal/waf/cve_node.go)
- [internal/waf/cve_php.go](file://internal/waf/cve_php.go)
- [internal/waf/cve_feed.go](file://internal/waf/cve_feed.go)
- [internal/admin/handler_cve.go](file://internal/admin/handler_cve.go)
- [internal/store/repository/cve_rule.go](file://internal/store/repository/cve_rule.go)
- [internal/waf/block.go](file://internal/waf/block.go)
- [internal/waf/drop.go](file://internal/waf/drop.go)
- [internal/waf/iprep_test.go](file://internal/waf/iprep_test.go)
- [internal/waf/bot_test.go](file://internal/waf/bot_test.go)
- [internal/waf/ratelimit_test.go](file://internal/waf/ratelimit_test.go)
- [internal/waf/owasp_test.go](file://internal/waf/owasp_test.go)
- [internal/waf/owasp_extended_test.go](file://internal/waf/owasp_extended_test.go)
- [internal/waf/fingerprint_test.go](file://internal/waf/fingerprint_test.go)
- [internal/waf/cve_test.go](file://internal/waf/cve_test.go)
</cite>

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [阻断机制详解](#阻断机制详解)
7. [依赖关系分析](#依赖关系分析)
8. [性能考虑](#性能考虑)
9. [故障排除指南](#故障排除指南)
10. [结论](#结论)
11. [附录](#附录)

## 简介
本文件系统性梳理 OpenWAF 的处理阶段实现，覆盖 IP 信誉阶段、ACL 阶段、机器人检测阶段、速率限制阶段、OWASP 检测阶段、CVE 检测阶段以及新增的 TLS 指纹识别组件。内容包括各阶段的输入输出、处理逻辑与决策机制、阶段间依赖与执行顺序、配置参数与性能调优建议，并提供测试方法与故障排除指南。

## 项目结构
OpenWAF 将请求处理抽象为"流水线（Pipeline）"，每个处理阶段封装为独立的 Phase 实现，按固定顺序串联执行。核心模块分布如下：
- 引擎层：负责站点解析、维护模式检查、规则编译与流水线组装
- 流水线层：定义统一的 Phase 接口与执行模型
- 规则阶段层：具体实现各处理阶段（IP 信誉、ACL、机器人检测、速率限制、OWASP、CVE、TLS 指纹识别等）
- WAF 核心算法层：IP 信誉、机器人检测、OWASP 检测、GeoIP 解析、速率限制、CVE 检测、TLS 指纹识别等

```mermaid
graph TB
Engine["Engine<br/>引擎"] --> Pipeline["Pipeline<br/>流水线"]
Pipeline --> Phase_IPRep["IP 信誉阶段"]
Pipeline --> Phase_ACL["ACL 阶段"]
Pipeline --> Phase_Bot["机器人检测阶段"]
Pipeline --> Phase_Rate["速率限制阶段"]
Pipeline --> Phase_OWASP["OWASP 检测阶段"]
Pipeline --> Phase_TLS["TLS 指纹识别阶段"]
Pipeline --> Phase_CVE["CVE 检测阶段"]
Pipeline --> Phase_Sig["签名阶段"]
Pipeline --> Phase_Custom["自定义阶段"]
```

**图表来源**
- [internal/core/engine/engine.go:57-129](file://internal/core/engine/engine.go#L57-L129)
- [internal/core/pipeline/pipeline.go:46-70](file://internal/core/pipeline/pipeline.go#L46-L70)

**章节来源**
- [internal/core/engine/engine.go:15-129](file://internal/core/engine/engine.go#L15-L129)
- [internal/core/pipeline/pipeline.go:9-70](file://internal/core/pipeline/pipeline.go#L9-L70)

## 核心组件
- 引擎 Engine：负责站点匹配、维护模式检查、规则编译与流水线构建
- 流水线 Pipeline：顺序执行各 Phase，支持 Drop/Intercept/Observe 等动作的短路与收集
- 规则阶段 Phase：各处理阶段的具体实现，如 IP 信誉、ACL、机器人检测、速率限制、OWASP、CVE、TLS 指纹识别、签名与自定义规则
- WAF 算法模块：IP 信誉、机器人检测、OWASP 检测、GeoIP、速率限制、CVE 检测、TLS 指纹识别等

**章节来源**
- [internal/core/engine/engine.go:15-176](file://internal/core/engine/engine.go#L15-L176)
- [internal/core/pipeline/pipeline.go:25-70](file://internal/core/pipeline/pipeline.go#L25-L70)
- [internal/core/rules/phases.go:32-358](file://internal/core/rules/phases.go#L32-L358)

## 架构总览
OpenWAF 的处理流程以 Engine 为中心，按以下顺序执行阶段：
1) IP 信誉阶段（白名单短路、黑名单拦截）
2) ACL 阶段（基于规则允许/拒绝）
3) 机器人检测阶段（两阶段：预筛选 + 深度评分；可选 GeoIP 加权）
4) 请求速率限制阶段（固定窗口计数）
5) OWASP 默认阶段（多类攻击检测）
6) TLS 指纹识别阶段（JA3/JA4 指纹分析、HTTP/2 设置校验、浏览器环境一致性检查）
7) CVE 检测阶段（针对特定漏洞模式，支持自动生成规则）
8) 签名与自定义阶段（通用规则匹配）

```mermaid
sequenceDiagram
participant Client as "客户端"
participant Engine as "Engine"
participant Pipe as "Pipeline"
participant IPRep as "IP 信誉阶段"
participant ACL as "ACL 阶段"
participant Bot as "机器人检测阶段"
participant RL as "速率限制阶段"
participant OWASP as "OWASP 检测阶段"
participant TLS as "TLS 指纹识别阶段"
participant CVE as "CVE 检测阶段"
participant Sig as "签名阶段"
participant Custom as "自定义阶段"
Client->>Engine : "请求进入"
Engine->>Engine : "站点解析/维护模式检查"
Engine->>Pipe : "构建阶段序列"
Pipe->>IPRep : "执行"
IPRep-->>Pipe : "允许/拦截/继续"
Pipe->>ACL : "执行"
ACL-->>Pipe : "允许/拦截/继续"
Pipe->>Bot : "执行可选 GeoIP 加权"
Bot-->>Pipe : "观察/拦截/继续"
Pipe->>RL : "执行"
RL-->>Pipe : "允许/拦截/继续"
Pipe->>OWASP : "执行"
OWASP-->>Pipe : "拦截/继续"
Pipe->>TLS : "执行"
TLS-->>Pipe : "拦截/继续"
Pipe->>CVE : "执行"
CVE-->>Pipe : "拦截/继续"
Pipe->>Sig : "执行"
Sig-->>Pipe : "拦截/继续"
Pipe->>Custom : "执行"
Custom-->>Pipe : "拦截/继续"
Pipe-->>Engine : "返回最终动作"
Engine-->>Client : "响应或阻断"
```

**图表来源**
- [internal/core/engine/engine.go:57-129](file://internal/core/engine/engine.go#L57-L129)
- [internal/core/rules/phases.go:32-358](file://internal/core/rules/phases.go#L32-L358)

**章节来源**
- [internal/core/engine/engine.go:57-129](file://internal/core/engine/engine.go#L57-L129)

## 详细组件分析

### IP 信誉阶段
- 输入：客户端 IP、站点配置的黑白名单、自动封禁阈值与窗口
- 输出：允许/拦截、是否命中、原因、分类（白名单/黑名单/自动封禁）
- 决策机制：
  - 先检查白名单，命中即允许并短路后续阶段
  - 再检查黑名单，命中即拦截
  - 若启用自动封禁且命中，则根据违规计数与封禁到期时间决定拦截
- 关键数据结构：
  - IPListEntry：支持单 IP 与 CIDR，带备注与过期时间
  - IPDecision：返回 Allowed/Mached/Reason/Category
  - violationCounter：记录违规次数、首次出现时间、封禁截止时间
- 性能特性：并发安全，定期清理过期封禁条目

```mermaid
flowchart TD
Start(["开始"]) --> CheckNil["IP 是否为空?"]
CheckNil --> |是| Allow["返回允许"]
CheckNil --> |否| ReadLock["读锁保护列表"]
ReadLock --> CheckWhitelist["遍历白名单"]
CheckWhitelist --> |命中| WhitelistAllow["返回允许并标记命中"]
CheckWhitelist --> |未命中| CheckBlacklist["遍历黑名单"]
CheckBlacklist --> |命中| BlacklistBlock["返回拦截并标记命中"]
CheckBlacklist --> |未命中| Unlock["释放读锁"]
Unlock --> CheckAutoBan["是否启用自动封禁?"]
CheckAutoBan --> |否| Pass["返回允许"]
CheckAutoBan --> |是| LoadViolation["加载违规计数器"]
LoadViolation --> HasBan{"bannedTil > 当前时间?"}
HasBan --> |是| AutoBanBlock["返回拦截并标记自动封禁"]
HasBan --> |否| Pass
```

**图表来源**
- [internal/waf/iprep.go:90-124](file://internal/waf/iprep.go#L90-L124)
- [internal/waf/iprep.go:126-155](file://internal/waf/iprep.go#L126-L155)

**章节来源**
- [internal/waf/iprep.go:18-124](file://internal/waf/iprep.go#L18-L124)
- [internal/core/rules/phases.go:130-170](file://internal/core/rules/phases.go#L130-L170)

### ACL 阶段
- 输入：客户端 IP、方法、路径、查询、头部映射
- 输出：允许/拦截/继续，命中描述与类别
- 决策机制：按规则匹配客户端 IP 或路径/查询模式，若命中则根据动作类型短路或继续

```mermaid
flowchart TD
Start(["开始"]) --> BuildCtx["构建匹配上下文"]
BuildCtx --> IterateRules["遍历 ACL 规则"]
IterateRules --> Match{"规则匹配?"}
Match --> |是| Decide["根据动作类型返回结果"]
Match --> |否| Next["继续下一个规则"]
Next --> IterateRules
Decide --> End(["结束"])
```

**图表来源**
- [internal/core/rules/phases.go:40-52](file://internal/core/rules/phases.go#L40-L52)
- [internal/waf/eval.go:87-114](file://internal/waf/eval.go#L87-L114)

**章节来源**
- [internal/core/rules/phases.go:32-52](file://internal/core/rules/phases.go#L32-L52)
- [internal/waf/eval.go:87-114](file://internal/waf/eval.go#L87-L114)

### 机器人检测阶段（两阶段：预筛选 + 深度评分）
- 输入：方法、路径、头部、UA、Cookie、连接方式、ClientIP
- 输出：BotVerdict（是否机器人、分数、类别、原因、规则 ID），以及 BotScore（深度评分明细）
- 预筛选（PreScreen）快速判定高风险：
  - 已知恶意工具 UA
  - 黑名单/自动封禁 IP
  - 高风险 GeoIP（数据中心/VPN/高危国家）
- 深度评分（DeepScore）综合评估：
  - GeoIP 风险评分与信息
  - UA/头部指纹评分
  - TLS/HTTP2 指纹评分
  - IP 信誉评分
- 决策机制：阈值决定拦截级别，恶意工具直接拦截，可疑行为仅记录观察

```mermaid
sequenceDiagram
participant Req as "请求"
participant Bot as "机器人检测阶段"
participant Pre as "预筛选"
participant Deep as "深度评分"
participant Geo as "GeoIP 解析器"
participant Rep as "IP 信誉"
Req->>Bot : "构造 BotRequest"
Bot->>Pre : "PreScreen()"
alt 已知恶意 UA 或黑名单/自动封禁
Pre-->>Bot : "高风险"
Bot->>Deep : "DeepScore()"
Deep->>Geo : "ScoreIP()/Lookup()"
Deep->>Rep : "Check(IP)"
Deep-->>Bot : "BotScore"
else 正常 UA
Pre-->>Bot : "低风险"
Bot-->>Bot : "跳过深度评分"
end
Bot-->>Req : "返回 BotVerdict + BotScore"
```

**图表来源**
- [internal/waf/bot.go:136-161](file://internal/waf/bot.go#L136-L161)
- [internal/waf/bot.go:167-224](file://internal/waf/bot.go#L167-L224)
- [internal/core/rules/phases.go:172-244](file://internal/core/rules/phases.go#L172-L244)

**章节来源**
- [internal/waf/bot.go:126-244](file://internal/waf/bot.go#L126-L244)
- [internal/core/rules/phases.go:172-244](file://internal/core/rules/phases.go#L172-L244)
- [internal/waf/geoip.go:153-223](file://internal/waf/geoip.go#L153-L223)

### 速率限制阶段（请求级）
- 输入：客户端 IP、Host（组合键）
- 输出：允许/拦截，命中描述与类别
- 决策机制：固定窗口计数，超过阈值即拦截；支持运行时重配置与禁用
- 关键点：错误率统计可单独使用增量接口（在上游响应后调用）

```mermaid
flowchart TD
Start(["开始"]) --> Enabled{"速率限制启用?"}
Enabled --> |否| Allow["允许通过"]
Enabled --> |是| GetKey["生成键: ClientIP|Host"]
GetKey --> AllowCheck["Allow(key)"]
AllowCheck --> |<=阈值| Allow
AllowCheck --> |>阈值| Block["拦截并短路"]
```

**图表来源**
- [internal/core/rules/phases.go:96-128](file://internal/core/rules/phases.go#L96-L128)
- [internal/waf/ratelimit.go:48-62](file://internal/waf/ratelimit.go#L48-L62)

**章节来源**
- [internal/core/rules/phases.go:96-128](file://internal/core/rules/phases.go#L96-L128)
- [internal/waf/ratelimit.go:9-117](file://internal/waf/ratelimit.go#L9-L117)

### OWASP 检测阶段
- 输入：路径、查询、头部、正文目标（按内容类型提取）
- 输出：OWASPHit 列表（类别、规则 ID、分数、描述）
- 检测范围：SQL 注入、XSS、命令注入、路径穿越、SSRF、XXE、LDAP 注入、NoSQL 注入、模板注入、JNDI 注入、CRLF、表达式语言、反序列化、协议违规、文件上传等
- 处理流程：
  - 收集扫描目标（路径、查询、头部、正文）
  - 清洗与归一化（URL/HTML/JS/UTF-7 解码、空白折叠、注释剥离）
  - 快速过滤（跳过明显安全的目标）
  - 多轮检测（基础规则 + 深度解码与 Base64 提取）
  - 协议级检测（头部层面）
  - 文件上传检测（扩展名与双扩展）
- 决策机制：命中即根据配置动作拦截或继续

```mermaid
flowchart TD
Start(["开始"]) --> Collect["收集目标: path/query/headers/body"]
Collect --> Normalize["归一化: URL/HTML/JS/UTF-7 解码"]
Normalize --> Clean{"是否安全?"}
Clean --> |是| NextTarget["下一个目标"]
Clean --> |否| Scan["扫描各类规则"]
Scan --> FP["误报抑制与上下文判断"]
FP --> Hits{"是否有命中?"}
Hits --> |是| Return["返回命中列表"]
Hits --> |否| DeepDecode["深度解码与 Base64 提取"]
DeepDecode --> Scan2["再次扫描"]
Scan2 --> FP2["误报抑制"]
FP2 --> Hits2{"是否有命中?"}
Hits2 --> |是| Return
Hits2 --> |否| Proto["协议级检测"]
Proto --> FileUpload["文件上传检测"]
FileUpload --> Return
```

**图表来源**
- [internal/waf/owasp.go:51-234](file://internal/waf/owasp.go#L51-L234)
- [internal/core/rules/phases.go:246-303](file://internal/core/rules/phases.go#L246-L303)

**章节来源**
- [internal/waf/owasp.go:48-384](file://internal/waf/owasp.go#L48-L384)
- [internal/core/rules/phases.go:246-303](file://internal/core/rules/phases.go#L246-L303)

### TLS 指纹识别阶段
- 输入：请求头部中的 TLS 指纹信息（JA3/JA4）、TLS 版本、HTTP/2 设置、用户代理、接受语言、接受编码、头部顺序
- 输出：FingerprintResult（风险评分、原因、匹配的指纹数据库条目）
- 检测机制：
  - JA3/JA4 指纹数据库匹配（恶意工具 vs 浏览器指纹）
  - TLS 版本异常检测（过低版本）
  - HTTP/2 设置一致性检查（SETTINGS、窗口大小）
  - 浏览器环境一致性验证（Accept-Language、Accept-Encoding 格式）
  - 头部顺序异常检测
- 决策机制：根据风险评分累加，超过阈值即拦截

```mermaid
flowchart TD
Start(["开始"]) --> Extract["ExtractFingerprint()"]
Extract --> CheckJA3["检查 JA3 指纹"]
CheckJA3 --> |恶意| Risk1["+40 分"]
CheckJA3 --> |已知浏览器| Risk2["检查浏览器匹配"]
Risk2 --> |不匹配| Risk3["+30 分"]
CheckJA3 --> |声称浏览器但未知| Risk4["+30 分"]
CheckJA3 --> |无| Next1["继续"]
Next1 --> CheckJA4["检查 JA4 指纹"]
CheckJA4 --> |仅 JA4| Risk5["+15 分"]
CheckJA4 --> |无| Next2["继续"]
Next2 --> CheckTLS["检查 TLS 版本"]
CheckTLS --> |< 1.2| Risk6["+15 分"]
CheckTLS --> |正常| Next3["继续"]
Next3 --> CheckH2["检查 HTTP/2 设置"]
CheckH2 --> |不一致| Risk7["+20 分"]
CheckH2 --> |一致| Next4["继续"]
Next4 --> CheckEnv["检查浏览器环境"]
CheckEnv --> |不一致| Risk8["+25 分"]
CheckEnv --> |一致| Next5["继续"]
Next5 --> CheckHeader["检查头部顺序"]
CheckHeader --> |异常| Risk9["+10 分"]
CheckHeader --> |正常| End(["结束"])
Risk1 --> End
Risk3 --> End
Risk4 --> End
Risk5 --> End
Risk6 --> End
Risk7 --> End
Risk8 --> End
Risk9 --> End
```

**图表来源**
- [internal/waf/fingerprint.go:46-96](file://internal/waf/fingerprint.go#L46-L96)
- [internal/waf/fingerprint.go:99-163](file://internal/waf/fingerprint.go#L99-L163)

**章节来源**
- [internal/waf/fingerprint.go:1-305](file://internal/waf/fingerprint.go#L1-L305)
- [internal/waf/fingerprint_db.go:1-233](file://internal/waf/fingerprint_db.go#L1-L233)

### CVE 检测阶段
- 输入：路径、查询、头部、正文、内容类型，支持自定义规则与外部威胁情报
- 输出：匹配的 CVE 条目（严重程度、类别、描述、规则 ID、动作）
- 检测架构：
  - 多技术栈专用检测器：PHP、Java、Node.js、通用规则
  - 自动规则生成：基于 NVD 和 GitHub Advisory 的威胁情报
  - 自定义规则管理：支持正则表达式、目标定位、启用状态
- 决策机制：并行执行各检测器，合并结果，优先使用配置动作，严重程度为 critical/high 自动提升为 Drop

```mermaid
flowchart TD
Start(["开始"]) --> BuildReq["BuildCVERequest()"]
BuildReq --> Parallel["并行检测"]
Parallel --> PHP["PHP 检测器"]
Parallel --> Java["Java 检测器"]
Parallel --> Node["Node.js 检测器"]
Parallel --> General["通用检测器"]
PHP --> Merge["合并结果"]
Java --> Merge
Node --> Merge
General --> Merge
Merge --> Custom["自定义规则匹配"]
Custom --> Action["应用动作"]
Action --> End(["结束"])
```

**图表来源**
- [internal/waf/cve_detector.go:76-109](file://internal/waf/cve_detector.go#L76-L109)
- [internal/waf/cve_detector.go:112-158](file://internal/waf/cve_detector.go#L112-L158)

**章节来源**
- [internal/waf/cve_detector.go:1-257](file://internal/waf/cve_detector.go#L1-L257)
- [internal/waf/cve_general.go:1-203](file://internal/waf/cve_general.go#L1-L203)
- [internal/waf/cve_java.go:1-130](file://internal/waf/cve_java.go#L1-L130)
- [internal/waf/cve_node.go:1-138](file://internal/waf/cve_node.go#L1-L138)
- [internal/waf/cve_php.go:1-194](file://internal/waf/cve_php.go#L1-L194)

## 阻断机制详解

OpenWAF 提供两种主要的阻断机制：HTTP 响应阻断和 TCP 连接阻断。

### HTTP 响应阻断（Block）
当请求被拦截时，系统会返回一个完整的 HTML 页面给客户端，显示阻断信息。这种阻断方式适用于大多数场景，因为它提供了友好的用户体验。

- **实现机制**：通过 `WriteBlockResponse` 函数渲染阻断页面
- **页面内容**：支持自定义 HTML 模板或使用内置的阻断页面
- **响应头**：设置 `X-Request-ID` 和 `X-WAF-Action` 头部
- **状态码**：默认 403，可配置

### TCP 连接阻断（Drop）
对于某些高风险攻击或需要立即终止连接的情况，系统提供 TCP 层面的阻断机制。

- **实现机制**：通过 `DropExecutor` 类实现，直接关闭 TCP 连接
- **性能优势**：不发送任何 HTTP 响应，立即终止连接
- **统计功能**：提供详细的阻断统计信息，包括按来源分类的阻断数量
- **日志记录**：记录每次阻断的详细信息，便于审计和分析

### 阻断决策流程

```mermaid
flowchart TD
Start(["请求处理"]) --> CheckAction["检查阶段动作"]
CheckAction --> IsDrop{"动作类型?"}
IsDrop --> |Drop| Drop["TCP 连接阻断"]
IsDrop --> |Intercept| Block["HTTP 响应阻断"]
IsDrop --> |Observe| Continue["继续处理"]
Drop --> Stats["更新阻断统计"]
Stats --> Log["记录阻断日志"]
Log --> End(["结束"])
Block --> Render["渲染阻断页面"]
Render --> End
Continue --> End
```

**图表来源**
- [internal/waf/block.go:17-39](file://internal/waf/block.go#L17-L39)
- [internal/waf/drop.go:61-83](file://internal/waf/drop.go#L61-L83)

**章节来源**
- [internal/waf/block.go:1-110](file://internal/waf/block.go#L1-L110)
- [internal/waf/drop.go:1-123](file://internal/waf/drop.go#L1-L123)

## 依赖关系分析
- Engine 依赖：站点解析器、IP 信誉、GeoIP 解析器、速率限制器、CVE 检测器、TLS 指纹识别器
- Pipeline 依赖：各 Phase 实现
- 各 Phase 依赖：WAF 算法模块（IP 信誉、机器人检测、OWASP、速率限制、GeoIP、CVE 检测、TLS 指纹识别）

```mermaid
graph TB
Engine["Engine"] --> Resolver["站点解析器"]
Engine --> IPRep["IP 信誉"]
Engine --> Geo["GeoIP 解析器"]
Engine --> RL["速率限制器"]
Engine --> CVE["CVE 检测器"]
Engine --> TLS["TLS 指纹识别器"]
Engine --> Pipe["Pipeline"]
Pipe --> Ph_IPRep["IP 信誉阶段"]
Pipe --> Ph_ACL["ACL 阶段"]
Pipe --> Ph_Bot["机器人检测阶段"]
Pipe --> Ph_Rate["速率限制阶段"]
Pipe --> Ph_OWASP["OWASP 检测阶段"]
Pipe --> Ph_TLS["TLS 指纹识别阶段"]
Pipe --> Ph_CVE["CVE 检测阶段"]
Pipe --> Ph_Sig["签名阶段"]
Pipe --> Ph_Custom["自定义阶段"]
```

**图表来源**
- [internal/core/engine/engine.go:16-36](file://internal/core/engine/engine.go#L16-L36)
- [internal/core/pipeline/pipeline.go:38-44](file://internal/core/pipeline/pipeline.go#L38-L44)

**章节来源**
- [internal/core/engine/engine.go:16-36](file://internal/core/engine/engine.go#L16-L36)
- [internal/core/pipeline/pipeline.go:38-44](file://internal/core/pipeline/pipeline.go#L38-L44)

## 性能考虑
- 并发与锁：IP 信誉与速率限制均采用互斥锁保护状态，注意在高并发下的竞争
- 计数与清理：IP 信誉与速率限制均有后台清理任务，避免内存无限增长
- 归一化成本：OWASP 归一化涉及多次解码与正则扫描，建议：
  - 控制目标长度（maxTargetLen）
  - 使用快速过滤跳过明显安全的目标
  - 限制正文扫描大小（如 multipart 字段值限制）
- GeoIP 查询：使用哈希集合进行 O(1) 快速判定，建议热更新配置而非频繁重载数据库
- 两阶段机器人检测：预筛选快速过滤，仅对高风险 IP 进行深度评分，降低 CPU 开销
- CVE 检测并行化：多检测器并行执行，使用 WaitGroup 收集结果，减少整体延迟
- TLS 指纹识别：指纹数据库内置缓存，头部顺序计算使用 MD5 哈希，性能开销可控
- 阻断机制：Drop 执行器提供高性能的 TCP 层阻断，适合高风险攻击场景

## 故障排除指南
- IP 信誉
  - 症状：白名单未生效、封禁未解除
  - 排查：确认列表格式（CIDR/单 IP）、备注字段、过期时间；检查自动封禁阈值与窗口设置
  - 测试参考：[internal/waf/iprep_test.go:9-92](file://internal/waf/iprep_test.go#L9-L92)
- 机器人检测
  - 症状：误判正常浏览器、漏判已知恶意工具
  - 排查：检查 UA 模式、GeoIP 配置、阈值设置；确认两阶段流程是否启用
  - 测试参考：[internal/waf/bot_test.go:5-79](file://internal/waf/bot_test.go#L5-L79)
- 速率限制
  - 症状：限流不生效或过度拦截
  - 排查：确认启用状态、窗口秒数与最大请求数；检查键生成（ClientIP|Host）
  - 测试参考：[internal/waf/ratelimit_test.go:7-43](file://internal/waf/ratelimit_test.go#L7-L43)
- OWASP 检测
  - 症状：误报/漏报
  - 排查：调整敏感度（low/mid/high）、检查误报抑制逻辑、确认头部跳过列表
  - 测试参考：[internal/waf/owasp_test.go:8-577](file://internal/waf/owasp_test.go#L8-L577)，[internal/waf/owasp_extended_test.go:8-471](file://internal/waf/owasp_extended_test.go#L8-L471)
- TLS 指纹识别
  - 症状：指纹未识别、误报高风险
  - 排查：确认上游代理正确传递 X-JA3-Hash、X-JA4-Hash、X-TLS-Version 等头部；检查浏览器指纹数据库完整性
  - 测试参考：[internal/waf/fingerprint_test.go:103-142](file://internal/waf/fingerprint_test.go#L103-L142)
- CVE 检测
  - 症状：未触发或动作不当
  - 排查：确认 CVE 开关、检测器初始化、严重程度与动作映射；检查自定义规则正则表达式有效性
  - 测试参考：[internal/waf/cve_test.go](file://internal/waf/cve_test.go)
- 阻断机制
  - 症状：阻断页面不显示或连接阻断无效
  - 排查：检查阻断页面模板、状态码配置；确认 Drop 执行器启用状态和统计功能

**章节来源**
- [internal/waf/iprep_test.go:9-119](file://internal/waf/iprep_test.go#L9-L119)
- [internal/waf/bot_test.go:5-79](file://internal/waf/bot_test.go#L5-L79)
- [internal/waf/ratelimit_test.go:7-43](file://internal/waf/ratelimit_test.go#L7-L43)
- [internal/waf/owasp_test.go:8-577](file://internal/waf/owasp_test.go#L8-L577)
- [internal/waf/owasp_extended_test.go:8-471](file://internal/waf/owasp_extended_test.go#L8-L471)
- [internal/waf/fingerprint_test.go:103-142](file://internal/waf/fingerprint_test.go#L103-L142)
- [internal/waf/cve_test.go](file://internal/waf/cve_test.go)

## 结论
OpenWAF 的处理阶段以流水线为核心，通过严格的阶段顺序与动作短路机制实现高效的安全防护。IP 信誉与 ACL 提供基础准入控制，机器人检测结合 GeoIP 与指纹技术提升识别精度，速率限制保障服务稳定性，OWASP 与 CVE 检测覆盖主流攻击面，TLS 指纹识别组件进一步增强了对自动化工具的检测能力。阻断机制提供了灵活的响应策略，既能在用户体验和性能之间取得平衡，又能应对高风险攻击场景。合理配置阈值与动作、优化归一化与扫描策略，可在准确性与性能之间取得最佳平衡。

## 附录

### 阶段配置参数与建议
- IP 信誉
  - 白名单/黑名单：支持 CIDR 与单 IP，建议明确备注与过期时间
  - 自动封禁：阈值（默认 10）、窗口（默认 60s）、持续时间（默认 3600s）
- 机器人检测
  - 阈值：默认 80；两阶段流程建议启用并结合 GeoIP
  - UA 模式与指纹评分：根据业务场景调整敏感度
- 速率限制
  - 窗口秒数与最大请求数：按 QPS 与业务峰值设定
  - 键策略：ClientIP|Host，避免跨站点共享
- OWASP
  - 敏感度：low/mid/high；建议从 mid 开始，逐步调整
  - 头部跳过：标准请求头（如 Host、Cookie、Authorization 等）默认跳过
- TLS 指纹识别
  - 指纹数据库：内置浏览器与工具指纹库，支持热更新
  - 风险阈值：默认 50 分，可根据环境调整
- CVE 检测
  - 动作：默认拦截，critical/high 自动 Drop
  - 规则来源：支持自定义规则与自动从威胁情报源同步
- 阻断机制
  - Block 页面：支持自定义 HTML 模板，可配置状态码
  - Drop 执行器：提供高性能的 TCP 层阻断，支持统计和日志记录

**章节来源**
- [internal/waf/iprep.go:68-79](file://internal/waf/iprep.go#L68-L79)
- [internal/waf/bot.go:399-454](file://internal/waf/bot.go#L399-L454)
- [internal/waf/ratelimit.go:24-46](file://internal/waf/ratelimit.go#L24-L46)
- [internal/waf/owasp.go:375-384](file://internal/waf/owasp.go#L375-L384)
- [internal/waf/fingerprint.go:264-277](file://internal/waf/fingerprint.go#L264-L277)
- [internal/waf/cve_detector.go:34-44](file://internal/waf/cve_detector.go#L34-L44)
- [internal/core/rules/phases.go:334-347](file://internal/core/rules/phases.go#L334-L347)
- [internal/waf/block.go:21-31](file://internal/waf/block.go#L21-L31)
- [internal/waf/drop.go:11-17](file://internal/waf/drop.go#L11-L17)

### 阶段测试方法
- IP 信誉：验证白/黑名单命中、自动封禁触发与封禁列表导出
  - 参考：[internal/waf/iprep_test.go:9-92](file://internal/waf/iprep_test.go#L9-L92)
- 机器人检测：验证已知恶意 UA、已知良好 UA、空请求、自动化库等场景
  - 参考：[internal/waf/bot_test.go:5-79](file://internal/waf/bot_test.go#L5-L79)
- 速率限制：验证启用/禁用、阈值越界、重配置生效
  - 参考：[internal/waf/ratelimit_test.go:7-43](file://internal/waf/ratelimit_test.go#L7-L43)
- OWASP：覆盖 SQLi/XSS/命令注入/路径穿越/SSRF/XXE/NoSQL 注入/模板注入/反序列化等
  - 参考：[internal/waf/owasp_test.go:8-577](file://internal/waf/owasp_test.go#L8-L577)，[internal/waf/owasp_extended_test.go:8-471](file://internal/waf/owasp_extended_test.go#L8-L471)
- TLS 指纹识别：验证恶意工具识别、浏览器指纹匹配、头部顺序异常检测
  - 参考：[internal/waf/fingerprint_test.go:103-142](file://internal/waf/fingerprint_test.go#L103-L142)
- CVE 检测：验证 PHP/Java/Node.js 漏洞检测、自定义规则匹配、威胁情报同步
  - 参考：[internal/waf/cve_test.go](file://internal/waf/cve_test.go)
- 阻断机制：验证 HTTP 响应阻断页面显示、TCP 连接阻断功能、统计信息记录
  - 参考：[internal/waf/block.go:17-39](file://internal/waf/block.go#L17-L39)，[internal/waf/drop.go:61-83](file://internal/waf/drop.go#L61-L83)

**章节来源**
- [internal/waf/iprep_test.go:9-119](file://internal/waf/iprep_test.go#L9-L119)
- [internal/waf/bot_test.go:5-79](file://internal/waf/bot_test.go#L5-L79)
- [internal/waf/ratelimit_test.go:7-43](file://internal/waf/ratelimit_test.go#L7-L43)
- [internal/waf/owasp_test.go:8-577](file://internal/waf/owasp_test.go#L8-L577)
- [internal/waf/owasp_extended_test.go:8-471](file://internal/waf/owasp_extended_test.go#L8-L471)
- [internal/waf/fingerprint_test.go:103-142](file://internal/waf/fingerprint_test.go#L103-L142)
- [internal/waf/cve_test.go](file://internal/waf/cve_test.go)
- [internal/waf/block.go:1-110](file://internal/waf/block.go#L1-L110)
- [internal/waf/drop.go:1-123](file://internal/waf/drop.go#L1-L123)

### CVE 规则管理 API
- 列表规则：支持分页、分类、严重程度、来源过滤
- 创建规则：支持正则表达式验证、自动标记来源
- 更新规则：支持部分字段更新、启用状态切换
- 删除规则：仅允许删除自定义规则
- 同步规则：手动触发威胁情报同步、查看同步状态

**章节来源**
- [internal/admin/handler_cve.go:15-217](file://internal/admin/handler_cve.go#L15-L217)
- [internal/store/repository/cve_rule.go:16-96](file://internal/store/repository/cve_rule.go#L16-L96)