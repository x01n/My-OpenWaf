...
FP["internal/waf/fingerprint.go"]
FPDB["internal/waf/fingerprint_db.go"]
CVE["internal/waf/cve_detector.go"]
CVEFEED["internal/waf/cve_feed.go"]
DROP["internal/waf/drop.go"]
END
subgraph "请求解析"
...
## 架构总览
WAF 在请求进入后，先解析客户端 IP，再依次评估 ACL、路径/查询阻断、IP 信誉、机器人检测、OWASP 规则、CVE 漏洞检测与速率限制，最后根据动作类型进行放行或阻断，并输出相应响应页与调试头。
