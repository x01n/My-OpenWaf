...
## 架构总览
WAF 引擎按阶段顺序执行：IPReputation → AntiReplay → ACL → OWASP 默认检测 → CVE 检测 → BotDetection → RequestRateLimit → Signature → Custom。缺少对应管理器或关闭配置时跳过相关阶段。每个阶段返回动作结果，Drop 最高优先级短路，Intercept 终止，Observe 收集日志。
