...
## 更新摘要
**变更内容**
- 新增完整的TLS指纹识别系统，包括JA3/JA4指纹检测
- 增强浏览器环境一致性检查机制
- 新增指纹统计分析功能和前端可视化界面
- 扩展指纹数据库，支持更多浏览器和恶意工具识别
- 增强HTTP/2指纹检测和头部顺序分析
...
- 解释机器人识别算法的实现原理：行为特征分析、请求模式识别、TLS指纹检测与指纹数据库匹配。
- 说明检测阈值的设置与调整方法：误报率控制与漏报率优化。
- 阐述不同类型的机器人检测策略：爬虫检测、DDoS 攻击检测与自动化工具检测。
- 解释检测结果的评估机制与反馈循环。
- 提供配置示例与性能调优建议，并说明与其他安全机制的协同工作方式。
- **新增**：详细介绍TLS指纹识别系统的实现，包括JA3/JA4指纹检测、浏览器环境一致性检查和指纹统计分析功能。

## 项目结构
机器人检测能力由后端 WAF 引擎与前端管理界面共同构成，现已扩展为包含完整的TLS指纹识别系统：
- 后端核心位于 internal/waf，包含两阶段检测（快速预筛选 + 深度评分）、指纹提取与评分、地理信息与信誉评估、速率限制等模块。
- **新增**：指纹数据库管理，支持浏览器指纹、恶意工具指纹和HTTP/2设置配置。
...
FPDB["指纹数据库<br/>internal/waf/fingerprint_db.go"]
...
- [handler_bot.go:144-153](file://internal/admin/handler_bot.go#L144-L153)
...
- **新增**：TLS指纹评分器（FingerprintScorer）
  - 提取 JA3/JA4、TLS 版本、HTTP/2 设置、头部顺序等特征，与内置指纹库比对，给出风险分数与原因。
  - 支持浏览器环境一致性检查、HTTP/2设置验证和头部顺序模式匹配。
- 地理与信誉
...
- **新增**：指纹统计分析
  - FingerprintStats：聚合JA3/JA4指纹统计、浏览器分布和异常告警。
  - 前端可视化界面：展示指纹Top列表、浏览器分布饼图和异常指纹告警。

**章节来源**
- [bot.go:126-454](file://internal/waf/bot.go#L126-L454)
- [fingerprint.go:30-163](file://internal/waf/fingerprint.go#L30-L163)
- [fingerprint_db.go:35-233](file://internal/waf/fingerprint_db.go#L35-L233)
- [geoip.go:26-111](file://internal/waf/geoip.go#L26-L111)
- [iprep.go:18-124](file://internal/waf/iprep.go#L18-L124)
- [eval.go:22-85](file://internal/waf/eval.go#L22-L85)
- [block.go:16-39](file://internal/waf/block.go#L16-L39)
...
## 架构总览
机器人检测在请求处理流程中的位置如下，现已集成了完整的TLS指纹识别系统：
...
- **浏览器JA3指纹**
  ...
  - Edge（基于Chrome）指纹
- **恶意工具JA3指纹**
  - curl、python-requests、Go http client、Java HttpClient等
  - wget、HTTPie、Node.js客户端、headless浏览器等
- **HTTP/2设置配置**
  - Chrome、Firefox、Safari、Edge的HTTP/2设置配置文件
  - 包括HeaderTableSize、EnablePush、MaxConcurrentStreams等参数
- **头部顺序模式**
  - 各浏览器典型头部顺序的MD5哈希模式
  - 用于检测异常的头部发送顺序
...
- [geoip.go:26-111](file://internal/waf/geoip.go#L26-L111)
- [iprep.go:18-124](file://internal/waf/iprep.go#L18-L124)
- [fingerprint_db.go:3-13](file://internal/waf/fingerprint_db.go#L3-L13)
...
- **后端统计接口**
  - GetFingerprints：获取指纹统计信息，包括Top JA3/JA4、浏览器分布和异常告警。
  - FingerprintStats：聚合统计数据结构，支持API返回。
- **前端可视化界面**
  - 指纹分析页面：展示JA3/JA4 Top列表、浏览器分布饼图和异常指纹告警。
  - 实时数据更新：通过API获取最新的指纹统计信息。
  - 异常告警：突出显示高风险的指纹模式和异常行为。
- **API集成**
  - 前端通过api.ts库调用GetFingerprints接口。
  - 支持认证令牌和错误处理机制。
  - 实时刷新和加载状态管理。
...
**图表来源**
- [handler_bot.go:144-153](file://internal/admin/handler_bot.go#L144-L153)
- [bot_score.go:91-133](file://internal/store/repository/bot_score.go#L91-L133)
- [fingerprints/page.tsx:37-177](file://frontend/app/(dashboard)/fingerprints/page.tsx#L37-L177)
...
- [fingerprints/page.tsx](file://frontend/app/(dashboard)/fingerprints/page.tsx#L37-177)
...
- 常见问题
  ...
  - **新增**：浏览器环境一致性检查误报：调整Accept-Language和Accept-Encoding格式。
- 排查步骤
...
- [fingerprint_test.go:7-33](file://internal/waf/fingerprint_test.go#L7-L33)

## 结论
该机器人检测系统以"两阶段快速预筛选 + 深度指纹评分"为核心，现已扩展为包含完整的TLS指纹识别系统，结合地理与信誉信息，形成多维度的风险画像。通过可配置的阈值与评分权重，可在误报与漏报之间取得平衡；配合速率限制与阻断页面，能够有效抵御爬虫、自动化扫描与 DDoS 攻击。**新增的指纹统计分析功能**提供了实时的指纹监控和异常检测能力，前端保护策略页面与后端配置模型打通，便于运维人员按需调整策略并观测效果。
...
#### JA3/JA4指纹技术
- **JA3指纹**：基于TLS ClientHello消息的哈希值，包含SSL版本、加密套件、扩展、签名算法等信息。
- **JA4指纹**：改进的JA3，包含更多TLS握手细节，提高唯一性和检测准确性。
- **指纹格式**：十六进制哈希字符串，用于快速匹配和检索。
- **应用场景**：识别特定的客户端库、浏览器版本和自动化工具。

#### HTTP/2指纹检测
- **SETTINGS帧分析**：检查HTTP/2 SETTINGS参数，如HeaderTableSize、InitialWindowSize等。
- **窗口大小验证**：验证初始窗口大小是否符合浏览器预期。
- **设置模式匹配**：与浏览器配置文件进行对比，识别异常设置。

#### 浏览器环境一致性检查
- **Accept-Language格式**：验证语言偏好格式是否符合浏览器预期。
- **Accept-Encoding支持**：检查压缩算法支持情况，如gzip、deflate、br等。
- **头部顺序模式**：验证HTTP头部发送顺序是否符合浏览器典型模式。

#### 指纹数据库结构
- **BrowserJA3**：浏览器指纹映射，键为JA3哈希，值为浏览器标识。
- **MaliciousJA3**：恶意工具指纹映射，键为JA3哈希，值为工具名称。
- **BrowserH2Settings**：HTTP/2设置配置文件，键为浏览器名称，值为设置参数。
- **HeaderOrderPatterns**：头部顺序模式，键为浏览器名称，值为MD5哈希列表。
