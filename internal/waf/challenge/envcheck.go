...
function loadWasm(){
 ...
 window.__owaf_wasm_ready=new Promise(function(resolve,reject){
  ...
  var script=document.createElement("script");script.src="/__owaf/pow_glue.js";script.async=true;script.onload=initialize;script.onerror=function(){reject(new Error("WASM glue load failed"))};(document.head||document.documentElement).appendChild(script);
 });
