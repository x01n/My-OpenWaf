...
func init() {
			...
			for _, s := range []string{uri, body, ua} {
				if reLog4j1.MatchString(s) || reLog4j2.MatchString(s) || reLog4j3.MatchString(s) ||
					...
					return &CVEMatch{
						...
						Category:    "cve_java",
						Severity:    "critical",
						Description: "通过混淆查找进行 Log4Shell JNDI 注入",
						MatchedPart: "all",
			...
			for _, v := range headers {
				if reLog4j1.MatchString(v) || reLog4j5.MatchString(v) {
					return &CVEMatch{
						...
						Category:    "cve_java",
						Severity:    "critical",
						Description: "通过 HTTP 请求头进行 Log4Shell JNDI 注入",
						MatchedPart: "header",
	...
	globalCVERuleRegistry.Register(&CVERule{
		...
		Name:     "Apache Tomcat Session 反序列化",
		CVE:      "CVE-2025-24813",
		Severity: "critical",
		Category: "cve_java",
		Enabled:  true,
		CheckFunc: func(uri, body, ua string, headers map[string]string) *CVEMatch {
			if reTomcat1.MatchString(uri) {
				return &CVEMatch{
					...
					Category:    "cve_java",
					Severity:    "critical",
					Description: "通过 .ser 文件路径进行 Apache Tomcat Session 反序列化",
					MatchedPart: "url",
	...
	d.rules = []javaCVERule{
		{
			...
			description: "通过 Log4j2 查找进行 Log4Shell JNDI 注入",
		...
		},
		{
			...
			description: "Spring4Shell 类加载器操纵 RCE",
		...
		},
		{
			...
			description: "通过 @type gadget 链进行 Fastjson 反序列化",
		...
		},
		{
			...
			description: "通过 Content-Type 或参数名进行 Struts2 OGNL 注入",
