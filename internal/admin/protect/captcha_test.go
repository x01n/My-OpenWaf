...
func TestCaptchaConfigPersistsShieldRuntimeFields(t *testing.T) {
	...
	if !loaded.ShieldRequireHTTP2 || loaded.ShieldRequireHTTP3 || loaded.ShieldAllowHTTP1 || !loaded.ShieldEnableJSChallenge || loaded.ShieldEnableEnvCheck || loaded.ShieldEnableDevTools {
...
func TestCaptchaConfigResponseIncludesShieldRuntimeFields(t *testing.T) {
	...
	for _, key := range []string{"shield_timeout_secs", "shield_auto_start_delay", "shield_max_retries", "shield_env_strictness", "shield_require_http2", "shield_allow_http1", "shield_enable_env_check", "shield_enable_devtools"} {
		if _, ok := got[key]; !ok {
