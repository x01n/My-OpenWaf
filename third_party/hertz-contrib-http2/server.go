...
func (sc *serverConn) maxHeaderListSize() uint32 {
	if n := sc.srv.MaxHeaderListSize; n > 0 {
		return n
	}
	...
	return uint32(http.DefaultMaxHeaderBytes + typicalHeaders*perFieldOverhead)
}
...
func (sc *serverConn) readFrames() {
	...
	for {
		if sc.engine.ReadTimeout != 0 {
			sc.conn.SetReadDeadline(time.Now().Add(sc.engine.ReadTimeout))
		}
...
func (sc *serverConn) scheduleFrameWrite() {
	...
	for !sc.writingFrameAsync {
		...
		if !sc.inGoAway || sc.goAwayCode == ErrCodeNo || sc.curOpenStreams() > 0 {
...
//
// TODO: configurable?
var goAwayTimeout = 3 * time.Second
...
func (sc *serverConn) processHeaders(f *MetaHeadersFrame) error {
	...
	} else if sc.srv.MaxHeaderFields > 0 && len(f.Fields) > sc.srv.MaxHeaderFields {
		st.handler = handleHeaderListTooLong
...
func (sc *serverConn) newStream(id, pusherID uint32, state streamState) *stream {
	...
	st.baseCtx, st.cancel = context.WithCancel(cc)
	st.cw.Init()
...
func writeResponseBody(rw *responseWriter, reqCtx *app.RequestContext) (err error) {
	...
	if reqCtx.Response.IsBodyStream() {
		...
		for {
			...
			if reqCtx.Response.ImmediateHeaderFlush {
				rw.Flush()
			}
		}
